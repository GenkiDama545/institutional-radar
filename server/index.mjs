...
const sockets=new Map(),desiredGroups=new Map(),activeGroups=new Map();let stopping=false;
function connect(index,ids){if(stopping)return;const ws=new WebSocket(socketUrl);sockets.set(index,ws);let lastMessage=Date.now(),opened=false;
  ...
  ws.addEventListener('close',()=>{clearInterval(ping);if(sockets.get(index)===ws){sockets.delete(index);activeGroups.delete(index)}if(opened)monitor.connections=Math.max(0,monitor.connections-1);if(!stopping)setTimeout(()=>{const latest=desiredGroups.get(index);if(latest&&!sockets.has(index))connect(index,latest)},5000+Math.random()*5000).unref()});
}
async function refresh(){try{
  const ids=await instruments(),plan=subscriptionPlan(ids,activeGroups);
  desiredGroups.clear();for(const [index,group] of plan.groups)desiredGroups.set(index,group);
  for(const index of plan.changed){const ws=sockets.get(index);if(ws)ws.close();else connect(index,plan.groups.get(index))}
  for(const index of plan.removed)sockets.get(index)?.close();
  console.log(`Tracking ${ids.length} live Spot USDT instruments`);
 }catch(e){console.error('Instrument refresh failed:',e)}}
