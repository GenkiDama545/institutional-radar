...
export class MarketMonitor {
  ...
  trade(row){
    ...
    const baseline=series.slice(0,-1).filter(b=>b.ts>=minute-21*60000).map(b=>b.usd).sort((a,b)=>a-b);
  ...
  publicState(){const fresh=this.lastTradeAt!==null&&this.now()-this.lastTradeAt<120000&&this.connections>0;return {fresh,lastTradeAt:this.lastTradeAt,connections:this.connections,coverage:this.bars.size,events:this.events.map(e=>({...e,live:fresh&&this.now()-e.updatedAt<120000}))}}
}
