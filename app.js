...
const APP_VERSION='V8.8.3';
// Public OKX market data does not prove that a contract is available to this account.
// Until X-Perp instruments can be matched exactly, only Spot can be actionable.
const ACCOUNT_VERIFIED_PERPS=new Set();
function accountCanTradePerp(id){return !!id&&ACCOUNT_VERIFIED_PERPS.has(id)}
...
function scenarioCandidates(){
 ...
 all.forEach(x=>{
  if(x.analysisCoverage==='complete'&&x.hasLongScenario&&x.longScore>=66){
   ...
   out.push({x,direction:'long',market:'spot',score:calibratedScore(x,'long'),kind:longKind,instruments:['spot']});
  }
...
function marketModeHint(){return 'Marchés du compte : seul le Spot est vérifié. Les contrats USDT-SWAP des données publiques OKX ne sont pas confirmés sur ton compte ; aucun scénario Perp n’est proposé.'}
...
async function scenarioHtml(){
 ...
 const primaryKind=proposedKind&&!isShortScenarioKind(proposedKind)?proposedKind:null;
 const primaryLabel=primaryKind?scenarioActionLabel(primaryKind):'AUCUN SCÉNARIO ACTUEL';
 const instrumentAvailability=accountCanTradePerp(current.perpId)?'Perp confirmé sur ce compte':'Perp public non vérifié sur ce compte • Spot uniquement';
...
function favoriteAsset(x){return {id:x.id,sym:x.sym,perpId:x.perpId||null,price:x.price,perpPrice:x.perpPrice,vol:x.vol,volUsd:x.volUsd,chg:x.chg,oi:x.oi,funding:x.funding,score:x.score,tier:x.tier,hasPerp:!!x.perpId,market:'spot'}}
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.8.2',{updateViaCache:'none'}).catch(console.warn));
