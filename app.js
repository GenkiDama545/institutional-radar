...
async function graphPage(){
 ...
 const instrument=activeInstrumentId(),seriesByBar=new Map();
 function indicatorSeries(cs){if(!seriesByBar.has(bar))seriesByBar.set(bar,RadarChartSeries.create(chartMath()));const provider=seriesByBar.get(bar);provider.update(raw,{asOf:Date.now()});return provider.aligned(cs)}
...
const scenarioChartSeries=new Map();
function chartMath(){return {ema,rsi,stochRsi,supertrend}}
async function setScenarioMonitorBar(kind,bar){
 if(!RadarChartCore.intervals[bar])return;
 scenarioMonitorBar=bar;const seq=++scenarioChartRequestSeq,page=pageRevision,snapshot=scenarioChartSnapshot;
 if(!snapshot)return;
 try{
  if(!snapshot.frames[bar]&&!scenarioChartFrames[bar]){
   const loaded=await candles(snapshot.instrument,bar,300);
   if(page!==pageRevision||seq!==scenarioChartRequestSeq||snapshot!==scenarioChartSnapshot)return;
   scenarioChartFrames[bar]=loaded;
  }
  renderScenarioChartOnly();
 }catch(err){if(page===pageRevision&&seq===scenarioChartRequestSeq)$('scenarioChartContent').innerHTML='<div class="empty">Consultation indisponible : '+esc(err.message)+'</div>'}
}
function scenarioChartContentHtml(){
 const s=scenarioChartSnapshot;if(!s)return '';
 const all=scenarioChartFrames[scenarioMonitorBar]||s.frames[scenarioMonitorBar]||[],cs=all.slice(-scenarioMonitorZoom);
 const reference=s.references?.[scenarioMonitorBar],mode=reference?.length?'engine':'historical',key=s.instrument+'|'+scenarioMonitorBar+'|'+mode;
 if(!scenarioChartSeries.has(key))scenarioChartSeries.set(key,RadarChartSeries.create(chartMath(),{reference:mode}));
 const provider=scenarioChartSeries.get(key);provider.update(all,{referenceBars:reference,asOf:s.asOf});
 const referenceLabel=mode==='engine'?'Référence moteur · analyse actuelle, bougies confirmées':'Consultation historique · ne reconstitue pas une ancienne décision';
 const tfBtns=Object.keys(RadarChartCore.intervals).map(b=>`<button class="smallbtn ${scenarioMonitorBar===b?'active':''}" onclick="setScenarioMonitorBar('${s.kind}','${b}')">${b}</button>`).join('');
 const zoomBtns=`<button class="smallbtn" aria-label="Afficher 30 bougies de moins" onclick="zoomScenarioMonitor('${s.kind}',-30)">−</button><button class="smallbtn" aria-label="Afficher 30 bougies de plus" onclick="zoomScenarioMonitor('${s.kind}',30)">＋</button><button class="smallbtn" onclick="resetScenarioChartView()">90 souhaitées</button>`;
 return `<div class="rangeBtns scenarioTfBtns">${tfBtns}</div><div class="rangeBtns scenarioZoomBtns">${zoomBtns}<span class="sub">${cs.length} bougies affichées</span></div><div class="sub">${referenceLabel}</div>${projectionChart(cs,s.lock,s.live,s.state,s.overlays,s.tickerFresh,{series:provider.aligned(cs)})}`;
}
function renderScenarioChartOnly(){if(scenarioChartSnapshot)$('scenarioChartContent').innerHTML=scenarioChartContentHtml()}
function resetScenarioChartView(){scenarioMonitorZoom=90;renderScenarioChartOnly()}
function zoomScenarioMonitor(kind,delta){scenarioMonitorZoom=Math.max(30,Math.min(240,scenarioMonitorZoom+delta));renderScenarioChartOnly()}
...
async function renderScenarioMonitor(kind,initial=false){kind=scenarioMonitorKind||kind;const page=pageRevision,seq=++scenarioMonitorSeq;const abandoned=()=>page!==pageRevision||seq!==scenarioMonitorSeq;try{
 if(initial){
  scenarioChartFrames={};scenarioChartRequestSeq++;scenarioChartSeries.clear();
 ...
 scenarioChartSnapshot=RadarChartCore.immutable({instrument:marketInstId,frames:scenarioMonitorFrames,kind:displayKind,lock:sc,live,state,overlays,tickerFresh,tickerTs,analysisAt:scenarioMonitorAnalysisAt,asOf:Date.now(),references:Object.fromEntries(Object.entries(e.F||{}).map(([tf,f])=>[tf,f?.cs||[]]))});
