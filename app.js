...
let all=[],current=null,filter='hot',marketMode='all',history=JSON.parse(localStorage.getItem('ir_hist_v52')||'{}'),theme='beige';
...
function scenarioCandidates(){
 const out=[];
 all.forEach(x=>{
  if(x.hasLongScenario&&x.longScore>=66)out.push({x,direction:'long',market:'spot',score:x.longScore});
  if(x.hasShortScenario&&x.shortScore>=66)out.push({x,direction:'short',market:'perp',score:x.shortScore});
 });
 return out;
}
...
function configCard(c,i,compact=false){const {x,direction,market,score}=c,tr=terrain(score),isShort=direction==='short';return `<div class="rankcard compactRank ${compact?'topRankCard':''}" onclick="openDetail('${x.id}')"><div class="rankTop"><span class="ranknum">${String(i+1).padStart(2,'0')}</span><b class="rankSym">${esc(x.sym)}</b><span class="tag ${market==='spot'?'b':'r'}">${market==='spot'?'💰 SPOT':'⚡ PERP'}</span><span class="tag ${isShort?'r':'g'}">${isShort?'📉 SHORT':'📈 LONG'}</span><span class="scoreBadge">${score}/100</span></div><div class="rankMetrics"><div class="metric"><small>Prix</small><b>${price(x.price)}</b></div><div class="metric"><small>24h</small><b class="${x.chg>=0?'good':'bad'}">${chg(x.chg)}</b></div><div class="metric"><small>Volume</small><b>${money(x.volUsd)}</b></div></div><div class="meter"><i style="width:${score}%"></i></div><div class="rankFoot"><span class="tag ${tr[1]}">${tr[0]}</span><span class="sub">🎯 Scénario valide</span></div></div>`}
function renderRank(){
 ...
 const filtered=candidates.filter(c=>marketMode==='all'||(marketMode==='spot'&&c.market==='spot')||(marketMode==='long'&&c.direction==='long')||(marketMode==='short'&&c.direction==='short')).filter(c=>c.x.bucket===filter).sort((a,b)=>b.score-a.score);
 $('rankTitle').textContent=`${bucketName(filter)} • ${modeLabel()}`;
 $('modeHint').textContent=marketModeHint();
 $('rank').innerHTML=filtered.length?`<div class="rank">${filtered.map((c,i)=>configCard(c,i)).join('')}</div>`:`<div class="empty">Aucune configuration ${marketMode==='short'?'SHORT / PERP ':marketMode==='spot'?'SPOT ':marketMode==='long'?'LONG ':''}dans ${bucketName(filter)} actuellement.<br><span class="sub">C’est volontaire : le Radar ne remplit pas une catégorie artificiellement.</span></div>`;
}
