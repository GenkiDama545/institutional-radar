...
async function scan(){
 ...
 try{
   ...
   raw.sort((a,b)=>b.score-a.score);raw.forEach(x=>x.bucket=bucket(x.score));localStorage.setItem(STORAGE.scan,JSON.stringify(history));all=raw;lastCompletedScanAt=Date.now();refreshScanFreshness();$('status').textContent=`${spotAssets.length} Spot vérifiés • ${discoveries.length} poussées repérées • ${all.filter(x=>x.analysisCoverage==='complete').length} analyses complètes • ${shortScanErrors} incomplètes`;render();
...
function renderRank(){
 ...
 const discovery=discoveries.filter(x=>!readyIds.has(x.id)&&(marketMode==='all'||(marketMode==='spot'&&x.market==='spot')||(marketMode==='long'&&x.minutePulse.priceMovePct>0)||(marketMode==='short'&&x.market==='xperp'&&x.minutePulse.priceMovePct<0))).sort((a,b)=>Math.abs(b.minutePulse.priceMovePct)*Math.min(20,b.minutePulse.ratio)-Math.abs(a.minutePulse.priceMovePct)*Math.min(20,a.minutePulse.ratio));
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.10',{updateViaCache:'none'}).catch(console.warn));
