...
function scenarioChartContentHtml(){
 ...
 return `<div class="rangeBtns scenarioTfBtns">${tfBtns}</div><div class="rangeBtns scenarioZoomBtns">${zoomBtns}<span class="sub">${cs.length} bougies affichées</span></div>${projectionChart(cs,s.lock,s.live,s.state,s.overlays,s.tickerFresh)}`;
}
...
async function renderScenarioMonitor(kind,initial=false){kind=scenarioMonitorKind||kind;const page=pageRevision,seq=++scenarioMonitorSeq;const abandoned=()=>page!==pageRevision||seq!==scenarioMonitorSeq;try{
 ...
 scenarioChartSnapshot=RadarChartCore.immutable({instrument:marketInstId,frames:scenarioMonitorFrames,kind:displayKind,lock:sc,live,state,overlays,tickerFresh,tickerTs,analysisAt:scenarioMonitorAnalysisAt});
...
function projectionChart(cs,sc,live,state,overlays=[],tickerFresh=false){let base=proChart(cs,cs,overlays);return `<div>${base}</div><div class="projectionLegend"><span class="projectionTag">🔒 Déclencheur</span><span class="projectionTag">🛑 Invalidation</span><span class="projectionTag">🟢 TP1/2/3</span><span class="projectionTag">${tickerFresh?'Ticker actualisé':'Dernier ticker connu'} ${axisPrice(live)}</span></div>`}
