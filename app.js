...
function paintSharedChart(id,input){
 ...
 if(!v){const instrument=input.instrument,page=pageRevision;v={host,controller:RadarChartHost.create(host,{math:chartMath(),loadHistory:async(tf,count)=>{const cs=await candles(instrument,tf,count);if(page!==pageRevision)throw Error('Consultation fermée');return cs}})};sharedChartViews.set(id,v);}
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.16',{updateViaCache:'none'}).catch(console.warn));
