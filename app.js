...
migrateStorage();
let shortScanErrors=0, scanErrorReasons={};let all=[],current=null,currentScenarioInstrument=null,filter='hot',marketMode='all',history=safeJSON(STORAGE.scan,{}),theme='beige';
...
async function get(path){await apiTurn();const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),12000);try{const r=await fetch(API+path,{signal:ctl.signal});if(!r.ok)throw Error('API '+r.status);const j=await r.json();if(j.code&&j.code!=='0')throw Error(j.msg||'API');return j.data||[]}catch(e){if(e?.name==='AbortError')throw Error('Délai API dépassé');throw e}finally{clearTimeout(timer)}}
...
async function scan(){
 ...
 try{
   ...
   await chunkRequests(raw.filter(x=>x.perpId),4,async x=>{try{const o=await get('/public/open-interest?instType=SWAP&instId='+encodeURIComponent(x.perpId));x.oi=n(o[0]?.oiUsd)}catch{x.oi=null}try{const f=await get('/public/funding-rate?instId='+encodeURIComponent(x.perpId));x.funding=n(f[0]?.fundingRate)}catch{x.funding=null}const h=history[x.id]||{samples:[]},prev=h.samples?.at(-1);x.oiDelta=prev&&Number.isFinite(prev.oi)&&prev.oi>0&&Number.isFinite(x.oi)?((x.oi-prev.oi)/prev.oi)*100:null;h.samples=[...(h.samples||[]),{ts:Date.now(),oi:x.oi,price:x.price,vol:x.vol,funding:x.funding,score:0}].slice(-192);history[x.id]=h});
   const deepUniverse=raw.slice(0,100);shortScanErrors=0;scanErrorReasons={};$('status').textContent=`Analyse technique de ${deepUniverse.length}/${raw.length} actifs…`;
   const chunk=chunkRequests;
   await chunk(deepUniverse,3,async x=>{try{const [cs1d,cs4,cs1h,cs30,cs15,cs5]=await Promise.all([candles(x.id,'1D',120),candles(x.id,'4H',100),candles(x.id,'1H',140),candles(x.id,'30m',140),candles(x.id,'15m',180),candles(x.id,'5m',180)]);const frames={'1D':cs1d,'4H':cs4,'1H':cs1h,'30m':cs30,'15m':cs15,'5m':cs5};x.scenarioFrames=frames;const f=timeframeFeatures(cs1h);if(f){const st=supertrend(cs1h,10,3).filter(Number.isFinite).at(-1),last=lastCompleted(cs1h)?.c||x.price;x.deep={trend:f.t,emaBias:f.t.e20>f.t.e50?'bull':f.t.e20<f.t.e50?'bear':'neutral',supertrendBias:st==null?'neutral':last>st?'bull':last<st?'bear':'neutral',rsi:f.rsi,stochK:f.stochK,stochD:f.stochD,adx:f.t.adx,bb:f.bb,atrPct:f.atr&&f.priceNow?f.atr/f.priceNow*100:null,compression:f.compression,expansion:f.expansion,priceAction:f.t.bull?'bullish':f.t.bear?'bearish':'neutral'}}x.scenarioModel=adaptiveEngine(frames,x);x.directional=x.scenarioModel?.directional||directionalAssessment(x.scenarioModel||{},x);x.hasLongScenario=!!(x.scenarioModel&&x.scenarioModel.longValid);x.hasShortScenario=!!(x.perpId&&x.scenarioModel&&x.scenarioModel.shortSetup&&(x.scenarioModel.breakdownValid||x.scenarioModel.rejectionValid));x.scenarioKind=x.scenarioModel?chooseFreshScenario(x.scenarioModel,null):null;x.hasValidScenario=!!(x.hasLongScenario||x.hasShortScenario)}catch(err){ shortScanErrors++;const reason=String(err?.message||err).slice(0,90);scanErrorReasons[reason]=(scanErrorReasons[reason]||0)+1;console.warn('Analyse '+x.id,err);x.deep=null;x.scenarioFrames={};x.scenarioModel=null;x.directional={longScore:0,shortScore:0,longEligible:false,shortEligible:false,strongest:'neutral'};x.scenarioKind=null;x.hasLongScenario=false;x.hasShortScenario=false;x.hasValidScenario=false}});
