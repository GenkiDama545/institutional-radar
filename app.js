...
function scenarioCandidates(){
 const out=[];
 all.forEach(x=>{
  if(x.hasLongScenario&&x.longScore>=66)out.push({x,direction:'long',market:'spot',score:x.longScore,kind:(x.scenarioModel?.directional?.longScore||0)>= (x.scenarioModel?.directional?.shortScore||0)?(scenarioValid('breakout',x.scenarioModel?.breakout,x.price)?'breakout':'pullback'):'pullback'});
  if(x.hasShortScenario&&x.shortScore>=66)out.push({x,direction:'short',market:'perp',score:x.shortScore,kind:x.scenarioModel?.rejectionValid?'rejection':'breakdown'});
 });
 return out;
}
...
function adaptiveEngine(frames,currentData){
 ...
 // V8.6.3 — LONG and SHORT are evaluated as two independent engines.
 // A bearish setup is not required to be a breakdown: a rejection/retest of resistance is also a valid short search pattern.
 ...
 const bearishContext=directional.shortEligible && !mtf.majorConflict;
 const bullishContext=directional.longEligible && !mtf.majorConflict;
 const longSetup=!!(bullishContext&&(nearR||nearS)&&!mtf.counter);
 const shortSetup=!!(bearishContext&&(shortNearR||shortNearS));
 const breakdownValid=shortSetup&&scenarioValid('breakdown',{...{entry:bdEntry,stop:bdStop,tp1:bdT1,tp2:bdT2,tp3:bdT3}},live);
 const rejectionValid=shortSetup&&scenarioValid('rejection',shortRejection,live);
...
function exportLearningJson(){const payload={schema:'IR_LEARNING_V1',exportedAt:new Date().toISOString(),app:'Institutional Radar V8.6.3',journal:loadScenarioJournal()};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='institutional-radar-learning-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
...
function scenarioMarket(kind){return isShortScenarioKind(kind)?'perp':'spot'}
...
function directionalScenarioLab(){
 const mk=(name,direction,score,nearR,nearS,live,entry,stop,tp1,kind,perp=true)=>({name,direction,score,nearR,nearS,live,entry,stop,tp1,kind,perp});
 const cases=[
  mk('Hausse forte','long',92,true,false,99,100,98,103,'breakout',false),
  mk('Baisse forte','short',92,false,true,101,100,103,97,'breakdown',true),
  mk('Range','none',50,false,false,100,101,99,102,'none',true),
  mk('Cassure haussière','long',88,true,false,99,100,98,103,'breakout',false),
  mk('Fausse cassure','none',58,true,false,100.5,100,98,103,'breakout',false),
  mk('Retournement baissier','short',84,true,false,101,100,103,97,'rejection',true),
  mk('Aucun setup','none',54,false,false,100,101,99,102,'none',true),
  mk('Marché surétendu','none',60,true,false,105,106,103,110,'breakout',false)
 ];
 const rows=cases.map(c=>{
  let result='NONE';
  if(c.direction==='long'&&c.score>=66&&scenarioValid('breakout',{entry:c.entry,stop:c.stop,tp1:c.tp1},c.live))result='LONG';
  if(c.direction==='short'&&c.perp&&c.score>=66&&scenarioValid(c.kind,{entry:c.entry,stop:c.stop,tp1:c.tp1},c.live))result='SHORT';
  const ok=(c.direction==='none'?result==='NONE':result===c.direction);
  return {name:c.name,expected:c.direction.toUpperCase(),result,ok};
 });
 return {rows,ok:rows.filter(x=>x.ok).length,total:rows.length};
}
function validationLabHtml(){const r=directionalScenarioLab();return `<div class="panel"><h2>🧪 Validation directionnelle V8.6.3</h2><div class="sub">Batterie minimale : tendance forte haussière, tendance forte baissière, range, breakout, false breakout, reversal, no setup et marché surétendu. Le test vérifie surtout que le moteur sait proposer LONG, SHORT ou refuser.</div><div class="grid">${r.rows.map(x=>`<div class="metric"><small>${esc(x.name)}</small><b class="${x.ok?'good':'bad'}">${x.ok?'✓':'✕'} ${x.result}</b><div class="foot">Attendu : ${x.expected}</div></div>`).join('')}</div><div class="callout ${r.ok===r.total?'goodbox':'badbox'}"><b>${r.ok}/${r.total} tests cohérents</b><br>${r.ok===r.total?'La batterie directionnelle ne force pas un LONG et sait produire un SHORT synthétique.':'La batterie signale encore un biais ou une condition à corriger avant adoption.'}</div></div>`}
