...
async function get(path){const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),12000);try{const r=await fetch(API+path,{signal:ctl.signal});if(!r.ok)throw Error('API '+r.status);const j=await r.json();if(j.code&&j.code!=='0')throw Error(j.msg||'API');return j.data||[]}catch(e){if(e?.name==='AbortError')throw Error('Délai API dépassé');throw e}finally{clearTimeout(timer)}}
...
function score(x){
  const m=sharedMarketSignals(x);
  let s=m.qualityScore;
  if(!m.complete)s-=4;
  if(Math.abs(x.funding||0)>.0005)s-=4;
...
function fmtMetric(v,u){v=n(v);if(!Number.isFinite(v))return 'N/D';if(u==='%')return (v*100).toFixed(3)+'%';if(u==='$')return Math.abs(v)<1?price(v)+' $':money(v);if(u==='score')return v.toFixed(0);return money(v)}
...
async function scan(){
 ...
 try{
   ...
   let raw=spotTickers.filter(x=>x.instId.endsWith('-USDT')).map(x=>{const last=n(x.last),open=n(x.open24h),vol=n(x.volCcy24h),sym=x.instId.replace('-USDT','');return {id:x.instId,spotId:x.instId,perpId:swaps.get(sym)?.instId||null,sym,market:'spot',hasPerp:!!swaps.get(sym),price:last,vol,volUsd:vol,chg:open>0?((last-open)/open)*100:0,high:n(x.high24h),low:n(x.low24h),oi:null,funding:null,oiDelta:null,rangePos:last&&n(x.high24h)>n(x.low24h)?(last-n(x.low24h))/Math.max(1e-12,n(x.high24h)-n(x.low24h)):null}}).filter(x=>x.vol>0&&x.price>0).sort((a,b)=>b.vol-a.vol).slice(0,150);
   const med=raw.reduce((s,x)=>s+x.vol,0)/Math.max(1,raw.length);raw.forEach(x=>x.medVol=med);
   $('universeInfo').textContent=`${raw.length} Spot actifs • dérivés associés quand disponibles`;
   await Promise.all(raw.map(async x=>{if(!x.perpId)return;try{const o=await get('/public/open-interest?instType=SWAP&instId='+encodeURIComponent(x.perpId));x.oi=n(o[0]?.oiUsd)}catch{x.oi=null}try{const f=await get('/public/funding-rate?instId='+encodeURIComponent(x.perpId));x.funding=n(f[0]?.fundingRate)}catch{x.funding=null}const h=history[x.id]||{samples:[]},prev=h.samples?.at(-1);x.oiDelta=prev&&Number.isFinite(prev.oi)&&prev.oi>0&&Number.isFinite(x.oi)?((x.oi-prev.oi)/prev.oi)*100:null;h.samples=[...(h.samples||[]),{ts:Date.now(),oi:x.oi,price:x.price,vol:x.vol,funding:x.funding,score:0}].slice(-192);history[x.id]=h}));
   const deepUniverse=raw.slice(0,80);$('status').textContent=`Analyse technique de ${deepUniverse.length}/${raw.length} actifs…`;
   const chunk=async(list,size,fn)=>{for(let i=0;i<list.length;i+=size)await Promise.all(list.slice(i,i+size).map(fn));};
   await chunk(deepUniverse,8,async x=>{try{const [cs1d,cs4,cs1h,cs30,cs15,cs5]=await Promise.all([candles(x.id,'1D',120),candles(x.id,'4H',100),candles(x.id,'1H',140),candles(x.id,'30m',140),candles(x.id,'15m',180),candles(x.id,'5m',180)]);const frames={'1D':cs1d,'4H':cs4,'1H':cs1h,'30m':cs30,'15m':cs15,'5m':cs5};x.scenarioFrames=frames;const f=timeframeFeatures(cs1h);if(f){const st=supertrend(cs1h,10,3).filter(Number.isFinite).at(-1),last=lastCompleted(cs1h)?.c||x.price;x.deep={trend:f.t,emaBias:f.t.e20>f.t.e50?'bull':f.t.e20<f.t.e50?'bear':'neutral',supertrendBias:st==null?'neutral':last>st?'bull':last<st?'bear':'neutral',rsi:f.rsi,stochK:f.stochK,stochD:f.stochD,adx:f.t.adx,bb:f.bb,atrPct:f.atr&&f.priceNow?f.atr/f.priceNow*100:null,compression:f.compression,expansion:f.expansion,priceAction:f.t.bull?'bullish':f.t.bear?'bearish':'neutral'}}x.scenarioModel=adaptiveEngine(frames,x);x.directional=x.scenarioModel?.directional||directionalAssessment(x.scenarioModel||{},x);x.hasLongScenario=!!(x.scenarioModel&&(x.scenarioModel.longSetup&&((scenarioValid('breakout',x.scenarioModel.breakout,x.price))||(scenarioValid('pullback',x.scenarioModel.pullback,x.price)))));x.hasShortScenario=!!(x.perpId&&x.scenarioModel&&(x.scenarioModel.shortSetup&&scenarioValid('breakdown',x.scenarioModel.breakdown,x.price)));x.scenarioKind=x.scenarioModel?chooseFreshScenario(x.scenarioModel,null):null;x.hasValidScenario=!!(x.hasLongScenario||x.hasShortScenario)}catch{ x.deep=null;x.scenarioFrames={};x.scenarioModel=null;x.directional={longScore:0,shortScore:0,longEligible:false,shortEligible:false,strongest:'neutral'};x.scenarioKind=null;x.hasLongScenario=false;x.hasShortScenario=false;x.hasValidScenario=false}});
   raw.forEach(x=>{x.signalModel=sharedMarketSignals(x);const base=score(x),dir=x.directional||{};x.longScore=dir.longScore||0;x.shortScore=dir.shortScore||0;x.side=x.hasShortScenario&&!x.hasLongScenario?'short':x.hasLongScenario&&!x.hasShortScenario?'long':x.longScore>x.shortScore?'long':x.shortScore>x.longScore?'short':'neutral';x.score=x.hasValidScenario?base:Math.min(base,47);x.tier=tier(x.vol,raw);let h=history[x.id];if(h?.samples?.length)h.samples[h.samples.length-1].score=x.score});
   raw.sort((a,b)=>b.score-a.score);raw.forEach(x=>x.bucket=bucket(x.score));localStorage.setItem('ir_hist_v63',JSON.stringify(history));all=raw;$('status').textContent=`${all.length} Spot actifs • ${all.filter(x=>x.hasPerp).length} avec perp`;render();
 }catch(e){$('status').textContent='Erreur API — réessaie';console.error(e)}
}
function render(){renderRank();drawTable()}
function renderRank(){
 const a=all.filter(x=>x.bucket===filter),long=a.filter(x=>x.longScore>=66).sort((x,y)=>y.longScore-x.longScore).slice(0,10),short=a.filter(x=>x.hasPerp&&x.shortScore>=66).sort((x,y)=>y.shortScore-x.shortScore).slice(0,10);
 const card=(x,i,side)=>{const sc=side==='long'?x.longScore:x.shortScore,tr=terrain(sc);return `<div class="rankcard compactRank" onclick="openDetail('${x.id}')"><div class="rankTop"><span class="ranknum">${String(i+1).padStart(2,'0')}</span><b class="rankSym">${esc(x.sym)}</b><span class="tag ${side==='long'?'g':'r'}">${side==='long'?'LONG':'SHORT'}</span><span class="scoreBadge">${sc}/100</span></div><div class="rankMetrics"><div class="metric"><small>Prix</small><b>${price(x.price)}</b></div><div class="metric"><small>24h</small><b class="${x.chg>=0?'good':'bad'}">${chg(x.chg)}</b></div><div class="metric"><small>Volume</small><b>${money(x.volUsd)}</b></div><div class="metric"><small>Perp</small><b>${x.hasPerp?'✓':'N/D'}</b></div><div class="metric"><small>Confluence</small><b>${x.score}/100</b></div><div class="metric"><small>Direction</small><b class="${side==='long'?'good':'bad'}">${sc}/100</b></div></div><div class="meter"><i style="width:${sc}%"></i></div><div class="rankFoot"><span class="sub">${esc(x.tier[1])}</span><span class="tag ${tr[1]}">${tr[0]}</span><span class="sub">${(side==='long'?x.hasLongScenario:x.hasShortScenario)?'🎯 Scénario valide':'⏳ Conditions'}</span></div></div>`};
 $('rank').innerHTML=`<div class="panel" style="margin:0;background:#0f1217"><div class="sectionTitle"><h2>🟢 LONG</h2><span class="sub">Scénarios acheteurs</span></div><div class="rank">${long.map((x,i)=>card(x,i,'long')).join('')||'<div class="empty">Aucun scénario LONG suffisamment construit dans cette catégorie.</div>'}</div></div><div class="panel" style="margin:0;background:#0f1217"><div class="sectionTitle"><h2>🔴 SHORT</h2><span class="sub">Scénarios vendeurs — perpétuels uniquement</span></div><div class="rank">${short.map((x,i)=>card(x,i,'short')).join('')||'<div class="empty">Aucun scénario SHORT suffisamment construit dans cette catégorie.</div>'}</div></div>`;
}
function drawTable(){let q=$('search').value.toUpperCase().trim(),s=$('sort').value,t=$('tier').value,a=all.filter(x=>(!q||x.sym.includes(q))&&(t==='all'||x.tier[0]===t));a.sort((x,y)=>s==='score'?y.score-x.score:s==='volume'?y.vol-x.vol:s==='momentum'?y.chg-x.chg:s==='oi'?y.oi-x.oi:s==='ratio'?y.oiRatio-x.oiRatio:Math.abs(y.funding)-Math.abs(x.funding));$('market').innerHTML=a.map((x,i)=>{let tr=terrain(x.score);return `<tr class="row" onclick="openDetail('${x.id}')"><td>${i+1}</td><td><b>${esc(x.sym)}</b><br><span class="tag">${x.tier[1]}</span></td><td>${price(x.price)}</td><td class="${x.chg>=0?'good':'bad'}">${chg(x.chg)}</td><td>${money(x.volUsd)}</td><td>${money(x.oi)}<br><span class="tiny ${x.oiDelta>=0?'good':'bad'}">${x.oiDelta==null?'1er scan':(x.oiDelta>=0?'+':'')+x.oiDelta.toFixed(1)+'% scan'}</span></td><td>${pct(x.funding)}</td><td><b>${x.score}</b><div class="meter"><i style="width:${x.score}%"></i></div></td><td><span class="tag ${tr[1]}">${tr[0]}</span></td></tr>`}).join('')||'<tr><td colspan="9" class="empty">Aucun résultat.</td></tr>'}
...
function proChart(cs,calcCs=cs,overlays=[]){
 ...
 const placed=[];validOverlays.forEach(o=>{const y=py(o.value);let ly=y;for(const q of placed.sort((a,b)=>a-b)){if(Math.abs(ly-q)<24)ly=q+(ly>=q?24:-24)}ly=Math.max(top+11,Math.min(top+priceH-11,ly));placed.push(ly);const col=o.color||'#65b8ff';const label=esc(o.label||'');svg.push(`<line x1="${L}" y1="${y}" x2="${W-R}" y2="${y}" stroke="${col}" stroke-width="2" stroke-dasharray="8 6" opacity=".95"/><rect x="5" y="${ly-11}" width="${L-12}" height="22" rx="6" fill="${col}" opacity=".98"/><text x="13" y="${ly+4}" fill="#06130a" font-size="12" font-weight="900">${label} ${axisPrice(o.value)}</text>`)});
 // candlesticks
 cs.forEach((c,i)=>{
...
function adaptiveEngine(frames,currentData){
 ...
 let triggerKey=null,best=-Infinity;for(const k of keys){let f=F[k],score=0;score+=(f.vol?.ratio>1?1:0)+(f.t.label===anchor.t.label?2:0)+(f.adx&&f.adx>18?1:0)+(f.eff&&f.eff>.25?1:0);score-=k==='1m'&&keys.length>3?.25:0;if(score>best){best=score;triggerKey=k}}
 const trigger=F[triggerKey], nearR=trigger.distR<=Math.max(trigger.atr*1.2,live*.004),nearS=trigger.distS<=Math.max(trigger.atr*1.2,live*.004);
 ...
 const longSetup=!!(directional.longEligible&&(nearR||nearS)&&!mtf.counter),shortSetup=!!(directional.shortEligible&&(nearR||nearS)&&!mtf.counter);
 ...
 return {F,anchorKey,triggerKey,anchor,trigger,live,mtf,regime:mtf,decision,shared,biasBull,biasBear,nearR,nearS,confluences,signals,directional,signalCounts:{positive:signals.filter(x=>x.status==='positive').length,negative:signals.filter(x=>x.status==='negative').length,warning:signals.filter(x=>x.status==='warning').length,neutral:signals.filter(x=>x.status==='neutral').length},score,readiness,levels:trigger.levels,breakout:{entry:brEntry,stop:brStop,tp1:brT1,tp2:brT2,tp3:brT3,risk:brRisk,rr:[(brT1-brEntry)/brRisk,(brT2-brEntry)/brRisk,(brT3-brEntry)/brRisk]},pullback:{entry:puEntry,stop:puStop,tp1:puT1,tp2:puT2,tp3:puT3,risk:puRisk,rr:[(puT1-puEntry)/puRisk,(puT2-puEntry)/puRisk,(puT3-puEntry)/puRisk]},breakdown:{entry:bdEntry,stop:bdStop,tp1:bdT1,tp2:bdT2,tp3:bdT3,risk:bdRisk,rr:[(bdT1?bdEntry-bdT1:0)/bdRisk,(bdT2?bdEntry-bdT2:0)/bdRisk,(bdT3?bdEntry-bdT3:0)/bdRisk]},longSetup,shortSetup,buffer:buf};
}
...
async function scenarioHtml(){
 ...
 const frames={};
 for(const [k,n] of [['1D',120],['4H',100],['1H',140],['30m',140],['15m',180],['5m',180]]){frames[k]=await safeLoad(k,n);if(current?.id!==targetId)throw Error('Analyse interrompue : actif changé');}
 let e=adaptiveEngine(frames,current); if(e)e.directional=directionalAssessment(e,current);
 ...
 ${current.hasPerp&&e.shortSetup&&scenarioValid('breakdown',e.breakdown,e.live)?scenarioCard('Cassure baissière','🔴',e.breakdown,'short',`clôture de ${e.triggerKey} sous ${axisPrice(e.breakdown.entry)} avec confirmation.`,shortNext,shortWhy,'breakdown'):''}
 ${!e.longSetup&&!e.shortSetup?`<div class="decision"><b>Pas de scénario directionnel suffisamment propre.</b><br>Le Radar préfère attendre plutôt que fabriquer une entrée.</div>`:''}
 </div>
...
function loadScenarioJournal(){try{return JSON.parse(localStorage.getItem('ir_scenario_journal_v86')||localStorage.getItem('ir_scenario_journal_v82')||'[]')}catch(_){return[]}}
function saveScenarioJournal(a){localStorage.setItem('ir_scenario_journal_v86',JSON.stringify(a.slice(-1000)))}
function learningSnapshot(e,kind){
 const d=current?.deep||{}; const dir=kind==='breakdown'?'short':'long';
 return {direction:dir,market:current?.market||'spot',hasPerp:!!current?.hasPerp,price:Number(current?.price)||null,chg24:Number(current?.chg)||null,vol:Number(current?.vol)||null,oi:Number(current?.oi)||null,oiDelta:Number.isFinite(current?.oiDelta)?current.oiDelta:null,funding:Number.isFinite(current?.funding)?current.funding:null,rangePos:Number.isFinite(current?.rangePos)?current.rangePos:null,deep:{rsi:Number.isFinite(d.rsi)?d.rsi:null,stochK:Number.isFinite(d.stochK)?d.stochK:null,adx:Number.isFinite(d.adx)?d.adx:null,atrPct:Number.isFinite(d.atrPct)?d.atrPct:null,emaBias:d.emaBias||'neutral',supertrendBias:d.supertrendBias||'neutral',priceAction:d.priceAction||'neutral',compression:!!d.compression,expansion:!!d.expansion},scores:{quality:e?.score??null,long:e?.directional?.longScore??null,short:e?.directional?.shortScore??null},mtf:e?.mtf?.rows||[],regime:e?.regime?.key||'unknown'}}
function scenarioFingerprint(e,kind){const m=e?.regime||{};const f=e?.F||{};const major=[f['1D'],f['4H'],f['1H']].map(x=>x?.t?.label||'mixte').join('/');return {regime:m.key||'unknown',kind,direction:kind==='breakdown'?'short':'long',anchor:e?.anchorKey||'N/D',trigger:e?.triggerKey||'N/D',mtfBias:e?.mtf?.bias||'neutral',mtfAgreement:e?.mtf?.agreement??null,major,score:e?.score??null,longScore:e?.directional?.longScore??null,shortScore:e?.directional?.shortScore??null,contradiction:e?.mtf?.contradiction??null}}
function journalCreate(e,kind,lock){const j=loadScenarioJournal();const fp=scenarioFingerprint(e,kind);const rec={id:lock.id,createdAt:lock.createdAt,closedAt:null,sym:current?.sym||current?.id||'N/D',instId:current?.id||'',kind,direction:kind==='breakdown'?'short':'long',market:current?.market||'spot',status:'FORMING',entry:lock.entry,stop:lock.stop,tp1:lock.tp1,tp2:lock.tp2,tp3:lock.tp3,score:lock.score,signals:lock.signals?.map(x=>({name:x.name,status:x.status,family:x.family}))||[],confluences:lock.confluences||[],fingerprint:fp,snapshot:learningSnapshot(e,kind),outcome:null};j.push(rec);saveScenarioJournal(j);return rec}
...
function journalOutcome(lock,side,live){const j=loadScenarioJournal(),rec=j.find(x=>x.id===lock.id);if(!rec||rec.status==='CLOSED')return;let outcome=null;if(side==='long'){if(live<=lock.stop)outcome={status:'SL',label:'Invalidé',price:live};else if(live>=lock.tp3)outcome={status:'TP3',label:'TP3 atteint',price:live};else if(live>=lock.tp2)outcome={status:'TP2',label:'TP2 atteint',price:live};else if(live>=lock.tp1)outcome={status:'TP1',label:'TP1 atteint',price:live}}else{if(live>=lock.stop)outcome={status:'SL',label:'Invalidé',price:live};else if(live<=lock.tp3)outcome={status:'TP3',label:'TP3 atteint',price:live};else if(live<=lock.tp2)outcome={status:'TP2',label:'TP2 atteint',price:live};else if(live<=lock.tp1)outcome={status:'TP1',label:'TP1 atteint',price:live}}if(outcome){const risk=Math.abs(lock.entry-lock.stop);const move=side==='short'?lock.entry-outcome.price:outcome.price-lock.entry;rec.status='CLOSED';rec.closedAt=Date.now();rec.outcome={...outcome,ts:Date.now(),r:risk>0?move/risk:null};rec.durationMs=Math.max(0,Date.now()-rec.createdAt);saveScenarioJournal(j)}}
function learningStats(){const j=loadScenarioJournal();const closed=j.filter(x=>x.status==='CLOSED'&&x.outcome);const groups={};for(const r of closed){const k=[r.direction||'unknown',r.market||'unknown',r.fingerprint?.regime||'unknown',r.kind||'unknown'].join(' / ');groups[k]??={n:0,tp1:0,tp2:0,tp3:0,sl:0,avgScore:0,avgR:0};const g=groups[k];g.n++;g.avgScore+=Number(r.score)||0;g.avgR+=Number(r.outcome.r)||0;if(r.outcome.status==='TP1')g.tp1++;if(r.outcome.status==='TP2')g.tp2++;if(r.outcome.status==='TP3')g.tp3++;if(r.outcome.status==='SL')g.sl++}Object.values(groups).forEach(g=>{g.avgScore=g.n?g.avgScore/g.n:0;g.avgR=g.n?g.avgR/g.n:0});return {total:j.length,closed:closed.length,groups,journal:j}}
function learningProposal(){const s=learningStats(),closed=s.journal.filter(x=>x.status==='CLOSED'&&x.outcome),byDir={long:[],short:[]};closed.forEach(r=>(byDir[r.direction]||byDir.long).push(r));const summarize=a=>{const n=a.length,sl=a.filter(r=>r.outcome.status==='SL').length,tp=a.filter(r=>/^TP/.test(r.outcome.status)).length;return {n,hitRate:n?tp/n:0,slRate:n?sl/n:0,avgR:n?a.reduce((z,r)=>z+(Number(r.outcome.r)||0),0)/n:0}};return {sample:closed.length,long:summarize(byDir.long),short:summarize(byDir.short),ready:closed.length>=30}} 
function exportLearningJson(){const payload={schema:'IR_LEARNING_V1',exportedAt:new Date().toISOString(),app:'Institutional Radar V8.6.2',journal:loadScenarioJournal()};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='institutional-radar-learning-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
...
function scenarioLockFromEngine(e,kind){
 ...
 return {id:crypto.randomUUID?crypto.randomUUID():String(Date.now()),kind,createdAt:Date.now(),anchorKey:e.anchorKey,triggerKey:e.triggerKey,entry:sc.entry,stop:sc.stop,tp1:sc.tp1,tp2:sc.tp2,tp3:sc.tp3,risk:sc.risk,rr:[...sc.rr],score:e.score,signals:e.signals.map(s=>({...s})),confluences:[...e.confluences]};
}
function chooseFreshScenario(e,preferred){
 const order=[preferred,'breakout','pullback','breakdown'].filter((x,i,a)=>x&&a.indexOf(x)===i);
 for(const k of order){if(e?.[k]&&scenarioValid(k,e[k],e.live)&&(k==='breakout'?e.longSetup:k==='pullback'?e.longSetup:e.shortSetup))return k}
 return null;
}
...
async function renderScenarioMonitor(kind,initial=false){kind=scenarioMonitorKind||kind;const seq=++scenarioMonitorSeq;try{
 if(initial){
  ...
  if(current?.id!==targetId)throw Error('Projection interrompue : actif changé');
 ...
 let live=current.price;try{let t=await get('/market/ticker?instId='+encodeURIComponent(current.id));if(Array.isArray(t)&&t[0]?.last)live=n(t[0].last)}catch(_){ }
 ...
 for(const b of refreshBars)scenarioMonitorFrames[b]=await candles(current.id,b,b==='1m'?300:180);
 const e=adaptiveEngine(scenarioMonitorFrames,{...current,price:live});if(seq!==scenarioMonitorSeq||!e)return;
 let actualKind=kind;const lockKey=scenarioLockKey(current.id,actualKind);let lock=scenarioLocks[lockKey];
 if(!lock){actualKind=chooseFreshScenario(e,kind);if(!actualKind){$('deepBody').innerHTML=`<button class="btn secondary" onclick="closeScenarioMonitor()">← Scénarios</button><div class="panel"><div class="monitorAction bad"><div class="actionTitle">🔴 AUCUNE CONFIGURATION VALIDE</div><div class="monitorSub">Le moteur refuse de verrouiller un scénario dont l'invalidation est déjà franchie.</div><button class="monitorBtn secondary" onclick="resetScenarioMonitor('${kind}')">🔎 Rechercher à nouveau</button></div></div>`;return}const nk=scenarioLockKey(current.id,actualKind);lock=scenarioLockFromEngine(e,actualKind);if(!lock)return;scenarioLocks[nk]=lock;scenarioMonitorKind=actualKind;journalCreate(e,actualKind,lock)}
 const triggerKey=lock.triggerKey||e.triggerKey,anchorKey=lock.anchorKey||e.anchorKey;
 if(!scenarioMonitorFrames[triggerKey])scenarioMonitorFrames[triggerKey]=await candles(current.id,triggerKey,180);
 if(scenarioMonitorBar!==triggerKey&&!scenarioMonitorFrames[scenarioMonitorBar])scenarioMonitorFrames[scenarioMonitorBar]=await candles(current.id,scenarioMonitorBar,300);
 ...
 const sc=lock,displayKind=lock.kind||actualKind,side=displayKind==='breakdown'?'short':'long';
 ...
 const jrec=loadScenarioJournal().find(x=>x.id===sc.id);if(jrec){if(activated&&jrec.status==='FORMING')journalUpdate(sc.id,{status:'ACTIVATED',activatedAt:Date.now()});journalOutcome(sc,side,live)}
 const overlays=[{value:sc.entry,color:'#65b8ff',label:'🎯 Déclencheur'},{value:sc.stop,color:'#ff6974',label:'🛑 Invalidation'},{value:sc.tp1,color:'#45dc7a',label:'TP1'},{value:sc.tp2,color:'#45dc7a',label:'TP2'},{value:sc.tp3,color:'#45dc7a',label:'TP3'},{value:live,color:'#ffd166',label:'Prix live'}];
 csForHover=chartCs;const chart=projectionChart(chartCs,sc,live,state,overlays);
 const action=state==='green'?`<div class="monitorAction ready"><div class="actionTitle">🎯 TRADER LE SCÉNARIO</div><div class="monitorSub">Le scénario verrouillé vient de satisfaire ses conditions. Les niveaux ci-dessous sont les paramètres hypothétiques calculés avant l'événement.</div><button class="monitorBtn" onclick="openScenarioSim('${current.id}',${sc.entry},${sc.stop},${sc.tp1},${sc.tp2},${sc.tp3},'${side}')">🧮 Préparer l'entrée dans le simulateur</button></div>`:state==='red'?`<div class="monitorAction bad"><div class="actionTitle">🔴 SCÉNARIO TERMINÉ</div><div class="monitorSub">Ne poursuis pas cette hypothèse. Le Radar peut rechercher une nouvelle configuration.</div><button class="monitorBtn secondary" onclick="resetScenarioMonitor('${displayKind}')">🔎 Rechercher une nouvelle configuration</button></div>`:`<div class="monitorAction ${state==='yellow'?'warn':''}"><div class="actionTitle">${state==='yellow'?'🟡 SURVEILLER LE DÉCLENCHEUR':'🟠 ATTENDRE LA CONFIRMATION'}</div><div class="monitorSub">${esc(reason)}</div></div>`;
...
function projectionChart(cs,sc,live,state,overlays=[]){let base=proChart(cs,cs,overlays);return `<div>${base}</div><div class="projectionLegend"><span class="projectionTag">🔒 Niveaux verrouillés</span><span class="projectionTag">Prix live ${axisPrice(live)}</span></div>`}
...
async function load(){
 ...
 try{
  if(['Prix','Volume','Momentum'].includes(metric)){
   cs=await candles(current.id,bar,calcLimit());
  ...
  let chartBlock=metric==='Prix'?proChart(cs):lineSvg(vals,fmt,times,metric==='Volume'?'Volume ($)':metric==='Funding'?'Funding (%)':metric==='Momentum'?'Variation (%)':metric==='OI'?'Open Interest ($)':metric==='Score'?'Score':'Valeur',days);body=`<button class="btn secondary" onclick="backDetail()">← ${current.sym}</button><div class="panel"><h2>📊 ${metric} — ${current.sym}</h2><div class="sub">${desc}</div>${rangeControls(bar,days)}<div id="metricMeta" class="sub" style="margin:8px 0">${bar} • ${days===1?'24H':days+'J'} • ${metric==='Prix'?cs.length:vals.length} ${metric==='Prix'?'bougies':'points'}</div>${chartBlock}</div><div class="panel"><h2>🧠 Lecture simple</h2><div class="decision"><b>Tendance récente : ${trend}.</b><br>${defs[metric]}</div><div class="accordion open"><div class="accHead">Définition <span>−</span></div><div class="accBody">${defs[metric]}</div></div><div class="accordion"><div class="accHead">Comment un trader le combine ? <span>＋</span></div><div class="accBody">Prix + volume + OI + funding + structure. On évite de décider à partir d'un seul indicateur.</div></div></div>`;
...
function btSlice(arr,ts,max=140){return arr.filter(c=>btTs(c)<=ts).slice(0,max)}
function btOutcome(cs,idx,kind,sc,horizon=18){
  const end=Math.min(cs.length-1,idx+horizon);let mfe=0,mae=0,hit=null;
  const entry=sc.entry,stop=sc.stop,tp1=sc.tp1;
  for(let j=idx+1;j<=end;j++){
    const h=Number(cs[j][2]),l=Number(cs[j][3]);
    if(kind==='breakdown'){
      mfe=Math.max(mfe,Math.max(0,entry-l));mae=Math.max(mae,Math.max(0,h-entry));
      if(h>=stop){hit='SL';break} if(l<=tp1){hit='TP1';break}
    }else{
      mfe=Math.max(mfe,Math.max(0,h-entry));mae=Math.max(mae,Math.max(0,entry-l));
      if(l<=stop){hit='SL';break} if(h>=tp1){hit='TP1';break}
    }
  }
  return {hit:hit||'TIMEOUT',mfe,mae};
}
function btScoreResult(rows){
  const closed=rows.filter(x=>x.outcome!=='TIMEOUT'), wins=rows.filter(x=>x.outcome==='TP1').length, sl=rows.filter(x=>x.outcome==='SL').length;
  const decided=wins+sl; return {n:rows.length,decided,wins,sl,timeout:rows.length-decided,rate:decided?wins/decided*100:0,avgScore:rows.length?rows.reduce((a,x)=>a+x.score,0)/rows.length:0};
}
async function runWalkForwardLab(){
  ...
  try{
    const inst=current?.id||'BTC-USDT-SWAP';
    ...
    for(const [phase,a,b] of phases){
      for(let i=Math.max(80,a);i<b;i+=Math.max(1,Math.floor((b-a)/18))){
        ...
        const px=btClose(base[i]);const e=adaptiveEngine(frames,{...(current||{}),price:px,oiDelta:null,funding:null});
        if(!e)continue;
    ...
    const cards=Object.entries(regime).sort((a,b)=>b[1].length-a[1].length).slice(0,12).map(([k,v])=>{const z=btScoreResult(v);return `<div class="metric"><small>${esc(k)}</small><b>${z.n} setups • ${z.rate.toFixed(0)}% TP1*</b><div class="foot">TP1 ${z.wins} • SL ${z.sl} • timeout ${z.timeout} • score ${z.avgScore.toFixed(0)}</div></div>`}).join('');
    const phaseCards=['DEV','VALIDATION','HOLDOUT'].map(p=>{const z=report[p];return `<div class="metric"><small>${p}</small><b>${z.n} setups</b><div class="foot">Décidés ${z.decided} • TP1 ${z.wins} • SL ${z.sl} • taux ${z.rate.toFixed(0)}% • score ${z.avgScore.toFixed(0)}</div></div>`}).join('');
    out.innerHTML=`<div class="panel"><h2>🧪 Résultat walk-forward</h2><div class="sub">${esc(inst)} • historique 1H comme horloge d'évaluation. Les scénarios sont reconstruits uniquement avec les bougies disponibles à chaque instant ; aucune bougie future n'est utilisée pour calculer le signal.</div><div class="grid">${phaseCards}</div></div><div class="panel"><h2>🧬 Par régime / configuration</h2><div class="grid">${cards||'<div class="empty">Aucun setup retenu sur cette fenêtre.</div>'}</div><div class="foot">* Taux TP1 calculé uniquement parmi les résultats décidés (TP1/SL), hors timeouts. Ce laboratoire n'est pas une promesse de performance et n'intègre pas ici frais, slippage, funding historique ni OI historique.</div></div><div class="panel"><h2>🛡️ Décision du laboratoire</h2><div class="callout goodbox"><b>Holdout intact.</b><br>Les résultats HOLDOUT sont affichés pour évaluation, mais cette exécution ne modifie aucune pondération du moteur. Toute nouvelle formule devra d'abord être proposée sur DEV, validée sur VALIDATION, puis testée sur un HOLDOUT jamais utilisé pour optimiser.</div></div>`;
