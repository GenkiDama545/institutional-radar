...
let all=[],current=null,currentScenarioInstrument=null,filter='hot',marketMode='all',history=JSON.parse(localStorage.getItem('ir_hist_v52')||'{}'),theme='beige';
...
function scenarioCandidates(){
 const out=[];
 all.forEach(x=>{
  if(x.hasLongScenario&&x.longScore>=66){
   ...
   const scoreAdj=market==='perp'?Math.max(0,x.longScore+Math.min(4,Math.max(0,x.oiDelta||0))):x.longScore;
   out.push({x,direction:'long',market,score:Math.min(100,Math.round(scoreAdj)),kind:longKind,instruments:x.hasPerp?['spot','perp']:['spot']});
  }
  if(x.hasShortScenario&&x.shortScore>=62&&x.perpId)out.push({x,direction:'short',market:'perp',score:x.shortScore,kind:x.scenarioModel?.shortPattern==='reversal'?'rejection':(x.scenarioModel?.shortPattern||'breakdown'),instruments:['perp']});
 });
 return out;
}
...
function shortEngineLab(){
 const cases=[
  {name:'SHORT — tendance baissière',expected:true,pattern:'trend',score:76},
  {name:'SHORT — rejet résistance',expected:true,pattern:'rejection',score:70},
  {name:'SHORT — fausse cassure',expected:true,pattern:'rejection',score:68},
  {name:'SHORT — retournement',expected:true,pattern:'reversal',score:67},
  {name:'AUCUN — range',expected:false,pattern:null,score:50}
 ];
 const rows=cases.map(c=>({name:c.name,expected:c.expected,result:!!c.pattern&&c.score>=62,ok:(!!c.pattern&&c.score>=62)===c.expected,score:c.score}));
