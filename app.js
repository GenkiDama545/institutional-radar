...
migrateStorage();
let shortScanErrors=0;let all=[],current=null,currentScenarioInstrument=null,filter='hot',marketMode='all',history=safeJSON(STORAGE.scan,{}),theme='beige';
...
function shortDiagnostics(){
 const a=all.filter(x=>x.perpId), modeled=a.filter(x=>x.scenarioModel), patterns=modeled.filter(x=>x.scenarioModel.shortPattern), eligible=patterns.filter(x=>x.scenarioModel.shortSetup), valid=eligible.filter(x=>x.hasShortScenario), listed=scenarioCandidates().filter(c=>c.direction==='short');
 return `SHORT : ${a.length} perps • ${modeled.length} analysés • ${patterns.length} motifs • ${eligible.length} scores éligibles • ${valid.length} scénarios valides • ${listed.length} affichés • ${typeof shortScanErrors==='number'?shortScanErrors:0} erreurs d'analyse`;
}
...
function scenarioCandidates(){
 ...
 all.forEach(x=>{
  ...
  if(x.perpId&&x.scenarioModel?.shortSetup&&x.shortScore>=62)out.push({x,direction:'short',market:'perp',score:x.shortScore,ready:x.hasShortScenario,kind:x.scenarioModel.shortPattern==='reversal'?'rejection':(x.scenarioModel.shortPattern||'breakdown'),instruments:['perp']});
 });
 return out;
}
