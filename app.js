 ...
 try{
   ...
   await chunk(deepUniverse,3,async x=>{
     try{
       ...
       if(x.market!=='xperp'&&full&&x.perpId&&x.perpPrice>0&&x.scenarioModel?.shortPattern&&x.directional.shortScore>=55){
   ...
   raw.sort((a,b)=>b.score-a.score);raw.forEach(x=>x.bucket=bucket(x.score));localStorage.setItem(STORAGE.scan,JSON.stringify(history));all=raw;lastCompletedScanAt=Date.now();refreshScanFreshness();$('status').textContent=`${all.length-xperps.length} Spot • ${xperps.length} X-Perps • ${all.filter(x=>x.analysisCoverage==='complete').length} analysés entièrement • ${shortScanErrors} incomplets`;render();
...
async function scenarioHtml(){
 ...
 const primaryKind=proposedKind&&(!isShortScenarioKind(proposedKind)||accountCanTradePerp(current.perpId))?proposedKind:null;
 const primaryLabel=primaryKind?scenarioActionLabel(primaryKind):'AUCUN SCÉNARIO ACTUEL';
 const instrumentAvailability=current.market==='xperp'?'X-Perp • '+current.id:accountCanTradePerp(current.perpId)?'Perp identifié':'USDT-SWAP public non vérifié • Spot uniquement';
