...
function renderRank(){
 ...
 const emptyReason=expired?'Les scénarios du dernier scan attendent une nouvelle validation du prix et du carnet. Relance le scan pour recalculer.':inMode.length?`${inMode.length} scénario(s) confirmé(s) dans ce sens, mais dans ${[...new Set(inMode.map(c=>bucketName(bucket(c.score))))].join(', ')}. Choisis cette catégorie pour les voir.`:marketMode==='short'?`Aucun SHORT confirmé sur les ${all.filter(x=>x.perpId).length} X-Perps publics analysés. Une baisse Spot reste une observation, pas une position SHORT exécutable.`:'Aucun scénario confirmé dans ce sens au dernier scan.';
...
async function analyzeDiscovery(x){
 ...
 x.hasLongScenario=!!x.scenarioModel?.longValid;x.hasShortScenario=!!(isListedXperp(x.perpId)&&x.scenarioModel?.shortSetup&&(x.scenarioModel.breakdownValid||x.scenarioModel.rejectionValid));
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.12',{updateViaCache:'none'}).catch(console.warn));
