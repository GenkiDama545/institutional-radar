...
function proChart(cs,calcCs=cs,overlays=[],presentation={}){
 ...
 const prepared=presentation.series,sri=prepared?null:stochRsi(calcCs);
 const e20=prepared?.ema20||ema(calcCs,20).slice(-cs.length),e50=prepared?.ema50||ema(calcCs,50).slice(-cs.length),st=prepared?.supertrend||supertrend(calcCs).slice(-cs.length),rv=prepared?.rsi||rsi(calcCs).slice(-cs.length),sv=prepared?.stochK||sri.k.slice(-cs.length),sd=prepared?.stochD||sri.d.slice(-cs.length);
 ...
 svg.push(`<text x="${L+5}" y="${Math.max(top,ly-22)+14}" fill="#06130a" font-size="10" font-weight="900">Prix bougie ${axisPrice(last)}</text>`);
...
function clearChartViews(){for(const v of sharedChartViews.values())v.controller.dispose();sharedChartViews.clear()}
...
function paintSharedChart(id,input){
 ...
 if(v&&v.host!==host){v.controller.dispose();sharedChartViews.delete(id);v=null;}
...
async function resetScenarioMonitor(kind){if(!clearScenarioLock(current.id,kind))return;scenarioMonitorKind=kind;scenarioMonitorFrames={};scenarioMonitorZoom=90;await renderScenarioMonitor(kind,true)}
...
function projectionChart(cs,sc,live,state,overlays=[],tickerFresh=false,presentation={}){let base=proChart(cs,cs,overlays,presentation);return `<div>${base}</div><div class="projectionLegend"><span class="projectionTag">🔒 Déclencheur</span><span class="projectionTag">🛑 Invalidation</span><span class="projectionTag">🟢 TP1/2/3</span><span class="projectionTag">${tickerFresh?'Ticker actualisé':'Dernier ticker connu'} ${axisPrice(live)}</span></div>`}
