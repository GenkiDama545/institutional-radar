...
function calibratedScore(x,side='best'){
  ...
  const direction=side==='long'?d.longScore:side==='short'?d.shortScore:Math.max(d.longScore||0,d.shortScore||0);
...
async function scan(){
 ...
 try{
   ...
   raw.forEach(x=>{x.signalModel=sharedMarketSignals(x);const dir=x.directional||{};x.longScore=dir.longScore||0;x.shortScore=dir.shortScore||0;x.side=x.hasShortScenario&&!x.hasLongScenario?'short':x.hasLongScenario&&!x.hasShortScenario?'long':x.longScore>x.shortScore?'long':x.shortScore>x.longScore?'short':'neutral';x.score=score(x);x.tier=tier(x.vol,raw);let h=history[x.id];if(h?.samples?.length)h.samples[h.samples.length-1].score=x.score});
...
function scenarioCandidates(){
 ...
 all.forEach(x=>{
  ...
  if(x.perpId&&x.scenarioModel?.shortSetup&&x.shortScore>=62)out.push({x,direction:'short',market:'perp',score:calibratedScore(x,'short'),ready:x.hasShortScenario,kind:x.scenarioModel.shortPattern==='reversal'?'rejection':(x.scenarioModel.shortPattern||'breakdown'),instruments:['perp']});
 });
 return out;
}
