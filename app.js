...
const STORAGE={scan:'ir_scan_history_v866',learning:'ir_learning_journal_v866'};
...
migrateStorage();
let all=[],current=null,currentScenarioInstrument=null,filter='hot',marketMode='all',history=safeJSON(STORAGE.scan,{}),theme='beige';
...
async function scan(){
 ...
 try{
   ...
   raw.sort((a,b)=>b.score-a.score);raw.forEach(x=>x.bucket=bucket(x.score));localStorage.setItem(STORAGE.scan,JSON.stringify(history));all=raw;$('status').textContent=`${all.length} Spot actifs • ${all.filter(x=>x.hasPerp).length} avec perp`;render();
...
function journalOutcome(lock,side,live){
 const j=loadScenarioJournal(),rec=j.find(x=>x.id===lock.id);if(!rec||rec.status==='CLOSED'||!Number.isFinite(live))return;
 let outcome=null;
 if(side==='long'){if(live<=lock.stop)outcome={status:'SL',label:'Invalidé',price:live};else if(live>=lock.tp3)outcome={status:'TP3',label:'TP3 atteint',price:live};else if(live>=lock.tp2)outcome={status:'TP2',label:'TP2 atteint',price:live};else if(live>=lock.tp1)outcome={status:'TP1',label:'TP1 atteint',price:live}}
 else{if(live>=lock.stop)outcome={status:'SL',label:'Invalidé',price:live};else if(live<=lock.tp3)outcome={status:'TP3',label:'TP3 atteint',price:live};else if(live<=lock.tp2)outcome={status:'TP2',label:'TP2 atteint',price:live};else if(live<=lock.tp1)outcome={status:'TP1',label:'TP1 atteint',price:live}}
 if(outcome){const risk=Math.abs(lock.entry-lock.stop),move=side==='short'?lock.entry-outcome.price:outcome.price-lock.entry;const ts=Date.now();rec.status='CLOSED';rec.closedAt=ts;rec.outcome={...outcome,ts,r:risk>0?move/risk:null};rec.durationMs=Math.max(0,ts-(rec.createdAt||ts));rec.governance={...(rec.governance||{}),phase:'OBSERVATION',closedBy:'LIVE_PRICE'};saveScenarioJournal(j)}
}
function learningStats(){
 const j=loadScenarioJournal();
 const closed=j.filter(x=>x.status==='CLOSED'&&x.outcome&&Number.isFinite(Number(x.outcome.r)));
 const groups={};
 ...
 return {total:j.length,closed:closed.length,open:j.filter(x=>x.status!=='CLOSED').length,groups,journal:j}
}
