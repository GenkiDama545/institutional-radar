...
async function runWalkForwardLab(){
  ...
  try{
    const inst=(currentScenarioInstrument==='perp'&&accountCanTradePerp(current?.perpId)?current.perpId:current?.id)||'BTC-USDT-SWAP';
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.0',{updateViaCache:'none'}).catch(console.warn));
