...
async function scan(){
 ...
 try{
   ...
   for(const x of additional){x.analysisCoverage='unavailable';x.perpAnalysisCoverage='unavailable';x.analysisIssues=['Analyse multi-horizon non encore réalisée'];const reference=x.market==='xperp'?perpReference:spotReference;x.medVol=reference.mean;x.marketMedianVol=reference.median;x.marketFresh=false}
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.13',{updateViaCache:'none'}).catch(console.warn));
