...
function accountCanTradePerp(id){return !!xperpBase(id)&&ACCOUNT_VERIFIED_XPERP_BASES.has(xperpBase(id))}
...
const minuteVolumePulse=RadarMarket.minuteVolumePulse;
function selectSpotForAnalysis(assets,limit=150){return RadarMarket.selectSpotForAnalysis(assets,favorites,limit)}
const isDiscovery=RadarMarket.isDiscovery;
...
function renderHostedFeed(){
 ...
 const live=hostedFeed.events.filter(e=>e.live).slice(0,8);
 const measured=hostedFeed.events.filter(e=>e.markouts?.[5]&&Date.now()-Number(e.detectedAt)<4*3600000).slice(0,8);
 const marks=e=>[5,15,60].filter(min=>e.markouts?.[min]).map(min=>`<span class="tag ${e.markouts[min].movePct>=0?'g':'r'}">+${min} min : ${chg(e.markouts[min].movePct)}${e.markouts[min].delayMs>60000?' (mesure tardive)':''}</span>`).join(' ');
 el.innerHTML=`<div class="sub">${hostedFeed.coverage} marchés observés • ${live.length} mouvement(s) récent(s) • observation sans scénario ni attribution à un acteur.</div>${live.length?`<div class="rank">${live.map(e=>`<div class="rankcard"><b>${esc(e.instId)}</b> <span class="tag y">À vérifier • Spot</span><div class="sub">Volume 1 min ×${Number(e.ratio).toFixed(1)} • prix ${chg(e.movePct)} • ${money(e.usd)} échangés</div><div class="sub">Détecté à ${timeLabel(Number(e.detectedAt))} • prix et carnet à revérifier</div>${marks(e)}</div>`).join('')}</div>`:'<div class="sub">Aucune poussée récente enregistrée.</div>'}${measured.length?`<details class="feedHistory"><summary>Voir le devenir des mouvements détectés (${measured.length})</summary><div class="sub">Variations observées après détection, sans frais ni simulation de trade ; un délai signifie que la première transaction disponible est arrivée plus tard.</div>${measured.map(e=>`<div class="feedObservation"><b>${esc(e.instId)}</b> · ${timeLabel(Number(e.detectedAt))}<div>${marks(e)}</div></div>`).join('')}</details>`:''}`;
}
async function scan(){
 ...
 try{
   const spotTickers=await get('/market/tickers?instType=SPOT');
   ...
   discoveries=minuteMarkets.filter(isDiscovery);
   let raw=selectSpotForAnalysis(spotAssets).concat(xperps);
   const volumes=raw.map(x=>x.vol).sort((a,b)=>a-b),med=raw.reduce((s,x)=>s+x.vol,0)/Math.max(1,raw.length),medianVol=volumes[Math.floor(volumes.length/2)]||med;raw.forEach(x=>{x.medVol=med;x.marketMedianVol=medianVol});
   $('universeInfo').textContent=`${spotAssets.length} Spot examinés en 1 min • ${raw.length-xperps.length} en analyse approfondie • ${xperps.length} X-Perps • ${minuteMissing} sans comparaison de volume • ${discoveries.length} poussées repérées`;
   await chunkRequests(xperps,4,async x=>{try{const o=await get('/public/open-interest?instType=FUTURES&instId='+encodeURIComponent(x.perpId));x.oi=nullableNumber(o[0]?.oiUsd);x.oiTs=x.oi==null?null:Date.now()}catch{x.oi=null}try{const f=await get('/public/funding-rate?instId='+encodeURIComponent(x.perpId));x.funding=nullableNumber(f[0]?.fundingRate);x.fundingTs=x.funding==null?null:Date.now()}catch{x.funding=null}});
   ...
   await chunk(deepUniverse,3,async x=>{
     try{
       ...
       x.hasShortScenario=!!(accountCanTradePerp(x.perpId)&&full&&x.perpScenarioModel?.shortSetup&&(x.perpScenarioModel.breakdownValid||x.perpScenarioModel.rejectionValid));
   ...
   const selectedIds=new Set(raw.map(x=>x.id));
   const additional=discoveries.filter(x=>!selectedIds.has(x.id));
   for(const x of additional){x.analysisCoverage='unavailable';x.perpAnalysisCoverage='unavailable';x.analysisIssues=['Analyse multi-horizon non encore réalisée'];x.medVol=med;x.marketMedianVol=med;x.marketFresh=false}
   raw=raw.concat(additional);
   ...
   raw.sort((a,b)=>b.score-a.score);raw.forEach(x=>{x.bucket=bucket(x.score);x.scenarioScanPrice=x.price});localStorage.setItem(STORAGE.scan,JSON.stringify(history));all=raw;lastCompletedScanAt=Date.now();lastReadyScenarios=scenarioCandidates().map(c=>({id:c.x.id,direction:c.direction,score:c.score}));const nextFilter=preferredFilter(lastReadyScenarios);if(nextFilter&&filter==='hot'&&!lastReadyScenarios.some(c=>bucket(c.score)==='hot'))setFilter(nextFilter);refreshScanFreshness();$('status').textContent=`${spotAssets.length} Spot vérifiés • ${discoveries.length} poussées repérées • ${all.filter(x=>x.analysisCoverage==='complete').length} analyses complètes • ${shortScanErrors} incomplètes`;render();
...
function shortDiagnostics(){
 ...
 return `SHORT : ${a.length} perps • ${modeled.length} analysés • ${patterns.length} motifs baissiers • ${eligible.length} scores éligibles • ${valid.length} scénarios confirmés • ${listed.length} cartes visibles (surveillance incluse) • ${shortScanErrors} analyses incomplètes${shortScanErrors?' ('+Object.entries(scanErrorReasons).sort((a,b)=>b[1]-a[1]).slice(0,2).map(([reason,count])=>count+'× '+reason).join(' ; ')+')':''}`;
}
...
async function refreshScenarioMarket(){
 if(marketRefreshRunning||scanRunning||!all.length||Date.now()-lastMarketRefreshAt<30000)return;
 marketRefreshRunning=true;lastMarketRefreshAt=Date.now();
 try{
  const assets=all.filter(x=>x.analysisCoverage==='complete'&&(x.hasLongScenario||x.hasShortScenario));
...
function configCard(c,i,compact=false){const {x,direction,market,score}=c,tr=terrain(score),isShort=direction==='short',evidence=evidenceForCard(x,direction);return `<div tabindex="0" role="button" class="rankcard compactRank ${compact?'topRankCard':''}" onclick="openDetail('${x.id}','${market}','${direction}')"><div class="rankTop"><span class="ranknum">${String(i+1).padStart(2,'0')}</span><b class="rankSym">${esc(x.sym)}</b><span class="tag ${market==='spot'?'b':'r'}">${market==='spot'?'💰 SPOT':'⚡ X-PERP'}</span><span class="tag ${isShort?'r':'g'}">${isShort?'🔴 SHORT':'🟢 LONG'}</span><span class="scoreBadge">${score}/100</span></div><div class="evidence"><span>✓ ${esc(evidence.reasons)}</span><span>À vérifier : ${esc(evidence.caution)}</span></div>${x.minutePulse?.recentUsd>=200&&x.minutePulse.ratio>=3?`<div class="sub">⚡ Flux récent (contexte, hors note) • volume 1 min ×${x.minutePulse.ratio===Infinity?'∞':x.minutePulse.ratio.toFixed(1)} • ${money(x.minutePulse.recentUsd)} récemment${x.volUsd<100000?' • faible liquidité 24 h':''}</div>`:''}${x.market==='xperp'?`<div class="sub" style="overflow-wrap:anywhere">${esc(x.id)}</div>`:''}<div class="rankMetrics"><div class="metric"><small>Prix</small><b>${price(isShort?x.perpPrice:x.price)}</b></div><div class="metric"><small>24h</small><b class="${x.chg>=0?'good':'bad'}">${chg(x.chg)}</b></div><div class="metric"><small>Volume</small><b>${money(x.volUsd)}</b></div></div><div class="meter"><i style="width:${score}%"></i></div><div class="rankFoot"><span class="tag ${tr[1]}">${tr[0]}</span><span class="sub">${c.ready===false?'⏳ Attendre le déclencheur':'🎯 Étudier les conditions'}</span></div></div>`}
function renderRank(){
 const candidates=scenarioCandidates();
 const readyIds=new Set(candidates.map(c=>c.x.id));
 const discovery=discoveries.filter(x=>!readyIds.has(x.id)&&(marketMode==='all'||(marketMode==='spot'&&x.market==='spot')||(marketMode==='long'&&x.minutePulse.priceMovePct>0)||(marketMode==='short'&&x.minutePulse.priceMovePct<0))).sort((a,b)=>Math.abs(b.minutePulse.priceMovePct)*Math.min(20,b.minutePulse.ratio)-Math.abs(a.minutePulse.priceMovePct)*Math.min(20,a.minutePulse.ratio));
 $('discovery').innerHTML=discovery.length?`<div class="sub">${discovery.length} poussée(s) repérée(s) hors classement des scénarios. Données au moment du scan.</div><div class="rank">${discovery.slice(0,discoveryExpanded?discovery.length:12).map(x=>`<div tabindex="0" role="button" class="rankcard" onclick="openDetail('${x.id}')"><b>${esc(x.sym)}</b> <span class="tag y">À analyser • aucun scénario classé</span><div class="sub">${x.market==='xperp'?'X-Perp':'Spot'} • volume 1 min ×${x.minutePulse.ratio===Infinity?'∞':x.minutePulse.ratio.toFixed(1)} • prix 1 min ${chg(x.minutePulse.priceMovePct)} • ${money(x.minutePulse.recentUsd)} échangés</div><div class="sub">${x.analysisCoverage==='complete'?executionCheck(x).reason:'Analyse multi-horizon à lancer en ouvrant la fiche'}${x.volUsd<100000?' • liquidité faible':''}</div></div>`).join('')}</div>${discovery.length>12?`<button class="btn secondary" onclick="discoveryExpanded=!discoveryExpanded;renderRank()">${discoveryExpanded?'Réduire la liste':'Voir les '+discovery.length+' cryptos détectées'}</button>`:''}`:'<div class="empty">Aucune poussée simultanée du prix et du volume hors scénarios actuellement analysés.</div>';
 ...
 const expired=lastReadyScenarios.length&&lastCompletedScanAt&&Date.now()-lastCompletedScanAt>120000&&!candidates.length;
 const bearish=marketMode==='short'?discoveries.filter(x=>x.minutePulse?.priceMovePct<0).sort((a,b)=>a.minutePulse.priceMovePct-b.minutePulse.priceMovePct).slice(0,5):[];
 const emptyReason=expired?'Les scénarios du dernier scan attendent une nouvelle validation du prix et du carnet. Relance le scan pour recalculer.':inMode.length?`${inMode.length} scénario(s) confirmé(s) dans ce sens, mais dans ${[...new Set(inMode.map(c=>bucketName(bucket(c.score))))].join(', ')}. Choisis cette catégorie pour les voir.`:marketMode==='short'?`Aucun SHORT confirmé sur les ${all.filter(x=>x.perpId).length} X-Perps reconnus. Une baisse Spot reste une observation, pas une position SHORT exécutable.`:'Aucun scénario confirmé dans ce sens au dernier scan.';
 $('rank').innerHTML=filtered.length?`<div class="rank">${filtered.map((c,i)=>configCard(c,i)).join('')}</div>`:`<div class="empty">Aucune configuration ${marketMode==='short'?'SHORT / PERP ':marketMode==='spot'?'SPOT ':marketMode==='long'?'LONG ':''}dans ${bucketName(filter)} actuellement.<br><span class="sub">${esc(emptyReason)}</span>${bearish.length?`<div class="sub" style="margin-top:12px">Baisses repérées, à étudier sans scénario SHORT confirmé :</div><div class="rank">${bearish.map(x=>`<div tabindex="0" role="button" class="rankcard" onclick="openDetail('${x.id}')"><b>${esc(x.sym)}</b> <span class="tag y">${x.market==='spot'?'Spot • observation baissière':'X-Perp • à vérifier'}</span><div class="sub">Prix 1 min ${chg(x.minutePulse.priceMovePct)} • volume ×${x.minutePulse.ratio===Infinity?'∞':x.minutePulse.ratio.toFixed(1)}</div></div>`).join('')}</div>`:''}</div>`;
}
...
async function scenarioHtml(){
 const sourceId=current.id,targetId=currentScenarioInstrument==='perp'&&accountCanTradePerp(current.perpId)?current.perpId:current.id;
 ...
 const primaryKind=execution.ok&&proposedKind&&(!isShortScenarioKind(proposedKind)||accountCanTradePerp(current.perpId))?proposedKind:null;
 ...
 const instrumentAvailability=current.market==='xperp'?'X-Perp • '+current.id:accountCanTradePerp(current.perpId)?'Perp identifié':'Spot uniquement';
 ...
 ${accountCanTradePerp(current.perpId)&&e.shortSetup&&e.rejectionValid?scenarioCard('Rejet / retest de résistance','🟠',e.shortRejection,'short',`rejet de la résistance ${axisPrice(e.levels.resistance)} puis clôture de ${e.triggerKey} sous ${axisPrice(e.shortRejection.entry)}.`,`Prochaine clôture ${e.triggerKey} : vérifier rejet + reprise de pression vendeuse.`,shortWhy,'rejection'):''}
 ${accountCanTradePerp(current.perpId)&&e.shortSetup&&e.breakdownValid?scenarioCard('Cassure baissière','🔴',e.breakdown,'short',`clôture de ${e.triggerKey} sous ${axisPrice(e.breakdown.entry)} avec confirmation.`,shortNext,shortWhy,'breakdown'):''}
 ${!e.longValid&&!accountCanTradePerp(current.perpId)?`<div class="decision"><b>Pas de scénario directionnel suffisamment propre.</b><br>Le Radar préfère attendre plutôt que fabriquer une entrée.</div>`:''}
 </div>
...
let favorites=safeJSON(STORAGE.favorites,{});
...
function toggleFavoriteAsset(id){const x=all.find(a=>a.id===id)||current;if(!x||x.id!==id)return;if(favorites[id])delete favorites[id];else favorites[id]={asset:favoriteAsset(x),scenarios:{}};saveFavorites();refreshFavoriteButtons();if($('favoritesBody'))renderFavorites()}
function toggleFavoriteScenario(id,kind){const x=all.find(a=>a.id===id)||current,lock=scenarioLocks[scenarioLockKey(id,kind)];if(!x||x.id!==id||!lock)return;const f=favorites[id]||{asset:favoriteAsset(x),scenarios:{}};f.scenarios||={};if(f.scenarios[kind]===lock.id)delete f.scenarios[kind];else f.scenarios[kind]=lock.id;favorites[id]=f;saveFavorites();refreshFavoriteButtons();if($('favoritesBody'))renderFavorites()}
...
async function openFavorite(id,kind=null){const f=favorites[id];if(!f)return;let x=all.find(a=>a.id===id);if(!x){x={...f.asset};try{const spot=await get('/market/ticker?instId='+encodeURIComponent(id));if(spot?.[0]?.last)x.price=n(spot[0].last);if(x.perpId){const perp=await get('/market/ticker?instId='+encodeURIComponent(x.perpId));if(perp?.[0]?.last)x.perpPrice=n(perp[0].last)}}catch(_){ }all.push(x)}current=x;if(kind){const lock=scenarioLocks[scenarioLockKey(id,kind)];if(!lock||f.scenarios?.[kind]!==lock.id)return;currentScenarioInstrument=lock.market;scenarioMonitorReturn='favorites';await openScenarioMonitor(id,kind)}else await openDetail(id)}
function renderFavorites(){const root=$('favoritesBody');if(!root)return;const journal=loadScenarioJournal(),items=Object.values(favorites);root.innerHTML=items.length?items.map(f=>{const x=f.asset,scenarios=Object.entries(f.scenarios||{}).map(([kind,lockId])=>{const lock=scenarioLocks[scenarioLockKey(x.id,kind)],rec=journal.find(r=>r.id===lockId),active=lock?.id===lockId&&(!isShortScenarioKind(kind)||accountCanTradePerp(x.perpId))&&(!rec||rec.status==='FORMING'||rec.status==='ACTIVATED');return `<div class="favoriteScenario"><b>${kind==='rejection'?'Rejet de résistance':kind==='breakdown'?'Cassure baissière':kind==='breakout'?'Cassure haussière':'Rebond sur support'} • ${rec?.status==='CLOSED'?'Terminé':rec?.status==='CANCELLED'?'Annulé':rec?.status==='UNVERIFIED'?'Suivi incomplet':active?rec?.status==='ACTIVATED'?'Activé':'En formation':isShortScenarioKind(kind)?'Contrat non vérifié sur ce compte':'Ancien scénario'}</b><div class="sub">Entrée ${price(lock?.entry??rec?.entry)} • Stop ${price(lock?.stop??rec?.stop)} • TP1 ${price(lock?.tp1??rec?.tp1)}</div>${active?`<button class="btn secondary" onclick="openFavorite('${esc(x.id)}','${esc(kind)}')">📈 Reprendre ce scénario</button>`:''}<button class="smallbtn" onclick="deleteFavoriteScenario('${esc(x.id)}','${esc(kind)}')">Retirer ce scénario des favoris</button></div>`}).join('');return `<div class="panel"><h3>★ ${esc(x.sym)}</h3><div class="sub">${esc(x.id)} • Favori enregistré sur cet appareil</div><div class="favoriteActions"><button class="btn" onclick="openFavorite('${esc(x.id)}')">Voir la crypto</button><button class="smallbtn" onclick="toggleFavoriteAsset('${esc(x.id)}')">Retirer la crypto</button></div>${scenarios}</div>`}).join(''):'<div class="panel"><div class="empty">Aucun favori pour le moment. Ajoute une crypto depuis son analyse ou un scénario depuis sa projection.</div></div>'}
function deleteFavoriteScenario(id,kind){if(!favorites[id])return;delete favorites[id].scenarios?.[kind];saveFavorites();renderFavorites()}
...
async function openScenarioMonitor(id,kind){
 ...
 if(isShortScenarioKind(kind)&&!accountCanTradePerp(current.perpId)){toolPage('favorites');$('deepBody').insertAdjacentHTML('afterbegin','<div class="panel"><b>Projection Perp non disponible</b><div class="sub">Ce contrat USDT-SWAP n’est pas confirmé dans ton compte OKX. Les anciens niveaux restent enregistrés comme historique, sans proposition de trade.</div></div>');return}
...
function closeScenarioMonitor(){if(scenarioMonitorTimer){clearInterval(scenarioMonitorTimer);scenarioMonitorTimer=null}scenarioMonitorSeq++;scenarioMonitorFrames={};scenarioMonitorKind=null;if(scenarioMonitorReturn==='favorites'){scenarioMonitorReturn='detail';toolPage('favorites')}else backDetail()}
...
async function renderScenarioMonitor(kind,initial=false){kind=scenarioMonitorKind||kind;const seq=++scenarioMonitorSeq;try{
 if(initial){
  const sourceId=current.id,targetId=isShortScenarioKind(kind)?current.perpId:(currentScenarioInstrument==='perp'&&accountCanTradePerp(current.perpId)?current.perpId:current.id);
 ...
 const marketInstId=isShortScenarioKind(kind)?current.perpId:(currentScenarioInstrument==='perp'&&accountCanTradePerp(current.perpId)?current.perpId:current.id);
...
function simForm(){let selected=current||all[0]||null,e=selected?.price||1,sl=e*.985,tp1=e*1.02,tp2=e*1.04,tp3=e*1.06;let opts=all.length?all.map(x=>`<option value="${esc(x.id)}" ${x.id===selected?.id?'selected':''}>${esc(bucketName(x.bucket))} — ${esc(x.sym)} — ${x.score}/100</option>`).join(''):`<option value="">Saisie manuelle • prix modifiables</option>`;return `<label>Crypto<select id="simCrypto" class="select" style="width:100%">${opts}</select></label><div class="simgrid" style="margin-top:8px"><label>Sens<select id="simSide" class="select"><option value="long">LONG</option><option value="short">SHORT</option></select></label><label>Capital (€)<input id="cap" class="input" type="number" value="100"></label><label>Risque max (%)<input id="riskPct" class="input" type="number" step="0.1" value="1"></label><label>Prix d'entrée<input id="entry" class="input" type="number" step="0.000000000001" value="${e}"></label><label>Levier x<input id="lev" class="input" type="number" step="0.1" value="1"></label><label>Stop / invalidation<input id="stop" class="input" type="number" step="0.000000000001" value="${sl.toPrecision(10)}"></label><label>Objectif 1<input id="tp1" class="input" type="number" step="0.000000000001" value="${tp1.toPrecision(10)}"></label><label>Objectif 2<input id="tp2" class="input" type="number" step="0.000000000001" value="${tp2.toPrecision(10)}"></label><label>Objectif 3<input id="tp3" class="input" type="number" step="0.000000000001" value="${tp3.toPrecision(10)}"></label></div><div id="simout" class="panel"></div>`}
function sim(){['cap','riskPct','entry','lev','stop','tp1','tp2','tp3','simSide'].forEach(id=>$(id)?.addEventListener('input',calc));$('simCrypto')?.addEventListener('change',e=>{current=all.find(x=>x.id===e.target.value)||current;if(!current)return;let p=current.price;$('entry').value=p;$('stop').value=(p*.985).toPrecision(10);$('tp1').value=(p*1.02).toPrecision(10);$('tp2').value=(p*1.04).toPrecision(10);$('tp3').value=(p*1.06).toPrecision(10);calc()});calc()}
...
function backDetail(){stopCandleStream();if(graphLiveTimer){clearInterval(graphLiveTimer);graphLiveTimer=null}if(scenarioMonitorTimer){clearInterval(scenarioMonitorTimer);scenarioMonitorTimer=null}$('deep').classList.add('hidden');$('detail').classList.remove('hidden')}
...
async function runWalkForwardLab(){
  ...
  try{
    const inst=(currentScenarioInstrument==='perp'&&accountCanTradePerp(current?.perpId)?current.perpId:current?.id)||'BTC-USDT';
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.11',{updateViaCache:'none'}).catch(console.warn));
