...
const APP_VERSION='V8.9.17-rc.1';
...
const STORAGE={scan:'ir_scan_history_v866',learning:'ir_learning_journal_v866',locks:'ir_scenario_locks_v871',favorites:'ir_favorites_v1',imports:'ir_import_archive_v1',transaction:'ir_state_transaction_v1'};
function safeJSON(key,fallback){try{const raw=stableStorageRaw(key);return raw?JSON.parse(raw):fallback}catch(_){return fallback}}
...
function transactionKeys(){return [STORAGE.scan,STORAGE.learning,STORAGE.locks,STORAGE.favorites,STORAGE.imports]}
function transactionRecord(){const raw=localStorage.getItem(STORAGE.transaction);if(!raw)return null;const tx=JSON.parse(raw);if(tx.schema!=='IR_TRANSACTION_V1'||!tx.before||!tx.after||Object.keys(tx.before).some(k=>!transactionKeys().includes(k))||Object.keys(tx.after).some(k=>!Object.hasOwn(tx.before,k)))throw Error('Transaction invalide');return tx}
function stableStorageRaw(key){const tx=transactionRecord();return tx&&Object.hasOwn(tx.before,key)?tx.before[key]:localStorage.getItem(key)}
function recoverStorageTransaction(){try{const tx=transactionRecord();if(!tx)return true;for(const [key,raw] of Object.entries(tx.before)){if(raw==null)localStorage.removeItem(key);else localStorage.setItem(key,raw)}localStorage.removeItem(STORAGE.transaction);return true}catch(_){return storageFailure()}}
function storageTransaction(updates){
 if(!recoverStorageTransaction())return false;
 let staged=false;
 try{
  const before={},after={};for(const [key,value] of Object.entries(updates)){if(!transactionKeys().includes(key))throw Error('Clé non autorisée');before[key]=localStorage.getItem(key);after[key]=JSON.stringify(value)}
  localStorage.setItem(STORAGE.transaction,JSON.stringify({schema:'IR_TRANSACTION_V1',before,after}));staged=true;
  for(const [key,raw] of Object.entries(after))localStorage.setItem(key,raw);
  localStorage.removeItem(STORAGE.transaction);return true;
 }catch(_){if(staged)recoverStorageTransaction();return storageFailure()}
}
function persistJSON(key,value){if(!recoverStorageTransaction())return false;try{localStorage.setItem(key,JSON.stringify(value));return true}catch(_){return storageFailure()}}
...
function migrateStorage(){if(!recoverStorageTransaction())return;try{
...
migrateStorage();
let shortScanErrors=0, scanErrorReasons={}, scanRunning=false;let all=[],current=null,currentScenarioInstrument=null,currentScenarioSelection=null,filter='hot',marketMode='all',history=scanHistory(),theme='beige';
...
function makeSpark(vals,times=[],options={}){return RadarChartMetrics.spark(vals,times,options)}
 ...
 try{
   ...
   await chunk(deepUniverse,3,async x=>{
     try{
       ...
       x.scenarioModel=sufficient?candidateModel(frames,x):null;
...
// One admission contract used by ranking, scenario cards and first lock creation.
async function decisionCandleRead(id,bar,count,timeout=7000){let timer;try{return await Promise.race([candles(id,bar,count),new Promise((_,reject)=>{timer=setTimeout(()=>reject(Error('timeout')),timeout)})])}catch(_){return []}finally{clearTimeout(timer)}}
function candidateModel(frames,source){
 const canonical=Object.fromEntries(DECISION_SPECS.map(([tf,n])=>[tf,(frames[tf]||[]).slice(-n)]));
 const e=adaptiveEngine(canonical,source);if(e)e.instrumentId=source.id;return e;
}
function scenarioKindAllowed(e,kind){
 if(!['breakout','pullback','breakdown','rejection'].includes(kind))return false;
 if(isShortScenarioKind(kind))return !!(e?.shortSetup&&e.directional?.shortScore>=62&&(kind==='rejection'?['rejection','reversal'].includes(e.shortPattern):e.shortPattern==='breakdown'));
 return !!(e?.longSetup&&e.directional?.longScore>=66);
}
function admitScenario(e,source,kind){
 if(!e||e.instrumentId!==source?.id)return {ok:false,reason:'Instrument du modèle différent du contrat sélectionné'};
 if(isShortScenarioKind(kind)&&(source.market!=='xperp'||source.id!==source.perpId||!isListedXperp(source.id)))return {ok:false,reason:'SHORT uniquement sur le X-Perp exact sélectionné'};
 const fresh=decisionFreshness(e,source);if(!fresh.ok)return fresh;
 const execution=executionCheck(source);if(!execution.ok)return execution;
 if(!scenarioKindAllowed(e,kind)||!scenarioValid(kind,scenarioLevelsFromEngine(e,kind),source.price))return {ok:false,reason:'Sens ou type sélectionné indisponible après recalcul'};
 return {ok:true,reason:execution.reason};
}
async function refreshDecisionSource(asset,id=asset.id){
 ...
 }catch(_){source.marketFresh=false;source.bookDepth=null}
 return source;
}
function scenarioCandidates(){
 const out=[];
 for(const x of all){const e=x.scenarioModel;
  for(const direction of ['long','short']){const kind=chooseFreshScenario(e,null,direction);if(!kind||!admitScenario(e,x,kind).ok)continue;
   out.push({x,direction,market:x.market==='xperp'?'perp':'spot',score:calibratedScore(x,direction),ready:true,kind,instruments:[x.market==='xperp'?'perp':'spot']});
  }
 }
 return out;
}
function shortBlockReason(x){
 ...
 const freshness=decisionFreshness(x.perpScenarioModel,x);if(!freshness.ok)return freshness.reason;
...
function subscribeCandleStream(instId,bar,onBar,onState){
 ...
 function connect(){
  ...
  socket.onmessage=e=>{if(e.data==='pong')return;let msg;try{msg=JSON.parse(e.data)}catch(_){return}if(msg.event==='error'){setState('Flux refusé par OKX · rafraîchissement toutes les 10 s');socket.close();return}if(msg.arg?.instId!==instId||msg.arg?.channel!=='candle'+bar)return;for(const a of msg.data||[]){const c={t:+a[0],o:+a[1],h:+a[2],l:+a[3],c:+a[4],v:+a[7],baseVol:+a[5],quoteVol:+a[7],confirm:+a[8]};if(RadarChartCore.bars([c]).length){lastMessageAt=Date.now();onBar(c)}}setState('● Bougies en direct · dernière réception '+clockStamp(lastMessageAt))};
...
const sharedChartViews=new Map();
let activeMetricPlot=null;
...
function paintSharedChart(id,input){
 ...
 if(v&&(v.host!==host||v.controller.input?.instrument!==input.instrument)){v.controller.dispose();sharedChartViews.delete(id);v=null;}
 if(!v){const instrument=input.instrument,page=pageRevision;v={host,controller:RadarChartHost.create(host,{math:chartMath(),tickerMaxAgeMs:RadarMarket.policy.tickerMaxAgeMs,loadHistory:async(tf,count)=>{const cs=await candles(instrument,tf,count);if(page!==pageRevision)throw Error('Consultation fermée');return cs}})};sharedChartViews.set(id,v);}
 return v.controller.paint(input);
}
async function chartTicker(instrument){try{const t=(await get('/market/ticker?instId='+encodeURIComponent(instrument)))[0],value=nullableNumber(t?.last),ts=nullableNumber(t?.ts);return {value,ts,fresh:value>0&&RadarMarket.freshTimestamp(ts)}}catch(_){return {value:null,ts:null,fresh:false}}}
async function graphPage(initialDays=7){
 const page=pageRevision,instrument=activeInstrumentId();let bar='1H',days=initialDays,drawSeq=0,raw=[],streamRevision=0,streamToken=0,ticker=null;
 const streamUpdates=new Map(),policy=RadarChartHost.policy;
 const limitForView=()=>Math.min(policy.maxLoad,Math.max(policy.minimumHistory,Math.ceil(days*86400000/timeframeMs(bar))+policy.warmup));
 function show(){
  if(page!==pageRevision||!$('garea'))return;
  const span=raw.length?(raw.at(-1).t-raw[0].t+timeframeMs(bar)):0,complete=span>=days*86400000;
  paintSharedChart('garea',chartSource(raw,bar,{instrument,ticker,notice:`${complete?'Période couverte':'Couverture partielle'} · ${days} jours demandés · ${raw.length} bougies disponibles (limite ${policy.maxLoad}).`}));
  $('gmeta').textContent=`${bar} · ${days} jours demandés · consultation historique · la vue initiale privilégie des bougies lisibles`;
 }
 function receive(c){if(page!==pageRevision||!RadarChartCore.bars([c]).length)return;streamUpdates.set(c.t,++streamRevision);if(streamUpdates.size>6000)streamUpdates.delete(streamUpdates.keys().next().value);const i=raw.findIndex(x=>x.t===c.t);if(i>=0)raw[i]=c;else raw.push(c);raw.sort((a,b)=>a.t-b.t);raw=raw.slice(-6000);show()}
 function connect(){stopCandleStream();const token=++streamToken;activeCandleStream=subscribeCandleStream(instrument,bar,c=>{if(token===streamToken)receive(c)},text=>{const el=$('liveState');if(page===pageRevision&&token===streamToken&&el)el.textContent=text})}
 async function draw(){const seq=++drawSeq,startedRevision=streamRevision,requestedBar=bar;try{
  const [data,nextTicker]=await Promise.all([candles(instrument,bar,limitForView()),chartTicker(instrument)]);
  if(page!==pageRevision||seq!==drawSeq||bar!==requestedBar||!$('garea')||$('deep').classList.contains('hidden'))return;
  ticker=nextTicker;
  if(data.length){const merged=new Map(data.map(c=>[c.t,c]));for(const c of raw)if(c.t>data.at(-1).t||(streamUpdates.get(c.t)||0)>startedRevision)merged.set(c.t,c);raw=[...merged.values()].sort((a,b)=>a.t-b.t).slice(-6000);show()}else if(!raw.length)show();
 }catch(err){if(page===pageRevision&&seq===drawSeq){const message='Actualisation impossible · '+String(err.message||err);$('liveState').textContent=message;sharedChartViews.get('garea')?.controller.error(message)}}}
 $('deepBody').innerHTML=`<button class="btn secondary" onclick="backDetail()">← ${esc(current.sym)}</button><div class="panel"><h2>📈 Graphique approfondi — ${esc(current.sym)}</h2><div class="sub">Consultation historique · flux de bougies OKX et actualisation REST toutes les 10 secondes. Le classement et les scénarios restent ceux du moteur.</div>${rangeControls(bar,days)}<div id="gmeta" class="sub">Chargement…</div><div id="liveState" class="liveStatus" role="status">Connexion…</div><button class="smallbtn" id="graphRefresh" type="button">↻ Recharger les bougies</button><div id="garea"></div><div id="graphSimulator">${scenarioSimPanel({id:'graph-free',free:true,entry:current.price},'long')}</div></div>`;
 bindScenarioSim();$('graphRefresh').onclick=draw;
 document.querySelectorAll('#bars button').forEach(b=>b.onclick=()=>{bar=b.dataset.bar;document.querySelectorAll('#bars button').forEach(x=>x.classList.toggle('active',x===b));raw=[];streamUpdates.clear();drawSeq++;connect();draw()});
 ...
 connect();await draw();if(page!==pageRevision||$('deep').classList.contains('hidden')||activeInstrumentId()!==instrument)return;if(graphLiveTimer)clearInterval(graphLiveTimer);graphLiveTimer=setInterval(()=>{if(document.visibilityState!=='hidden')draw()},10000);
}
...
async function scenarioHtml(){
 ...
 const safeLoad=(k,n)=>decisionCandleRead(targetId,k,n);
 // Six horizons required for admission; partial readings cannot create a lock.
 const frames={};
 for(const [k,n] of DECISION_SPECS){frames[k]=await safeLoad(k,n);if(page!==pageRevision||current?.id!==sourceId)throw Error('Analyse interrompue : actif changé');}
 const source=await refreshDecisionSource(current,targetId);if(page!==pageRevision)return '';const e=candidateModel(frames,source);
 
 if(!e){if(scenarioCandidates().some(candidate=>candidate.x.id===current.id)){current.marketFresh=false;renderRank()}return `<div class="panel"><div class="empty">Le scénario du classement n’est plus confirmé : bougies insuffisantes après actualisation. Cette crypto a été retirée du Top. Relance le scan pour recalculer.<br><span class="sub"><br><span class="sub">Données insuffisantes ; six horizons valides sont requis pour une décision.</span></div></div>`}
 ...
 const requested=currentScenarioSelection?.id===current.id?currentScenarioSelection:null;
 const proposedKind=chooseFreshScenario(e,requested?.kind,requested?.direction);
 const execution=proposedKind?admitScenario(e,source,proposedKind):{ok:false,reason:'Le sens/type sélectionné est indisponible après recalcul ; aucune substitution.'};
 const primaryKind=execution.ok?proposedKind:null;
 const cardAllowed=kind=>(!requested?.kind||requested.kind===kind)&&(!requested?.direction||requested.direction===scenarioDirection(kind))&&admitScenario(e,source,kind).ok;
 ...
 const breakoutNext=`Prochaine clôture ${e.triggerKey} : vérifier seuil, clôture, volume ≥1,15× et biais.`;
 const pullNext=`Prochaine clôture ${e.triggerKey} : vérifier seuil, clôture, volume ≥1,15× et biais.`;
 ...
 const pullWhy=`support ${axisPrice(e.levels.support)}; hypothèse de support, sans automate de défense ou retest.`;
 ...
 <div class="panel"><h2>🎯 Scénarios chiffrés</h2><div class="sub">Règle commune : prix et clôture au-delà du seuil, volume ≥1,15× et biais compatible. Les noms décrivent des hypothèses de structure ; aucun automate de retest n’est exécuté. Clique dessus pour afficher la projection sur les bougies et suivre le déclencheur.</div>
 ${cardAllowed('breakout')?scenarioCard('Cassure conditionnelle','🟢',e.breakout,'long',`clôture de ${e.triggerKey} au-dessus de ${axisPrice(e.breakout.entry)} avec volume ≥1,15× et biais compatible.`,breakoutNext,breakoutWhy,'breakout'):''}
 ${cardAllowed('pullback')?scenarioCard('Rebond sur support','🟡',e.pullback,'long',`prix et clôture ${e.triggerKey} au-dessus de ${axisPrice(e.pullback.entry)}, volume ≥1,15× et biais compatible.`,pullNext,pullWhy,'pullback'):''}
 ${cardAllowed('rejection')?scenarioCard('Rejet conditionnel de résistance','🟠',e.shortRejection,'short',`prix et clôture ${e.triggerKey} sous ${axisPrice(e.shortRejection.entry)}, volume ≥1,15× et motif vendeur compatible.`,`Prochaine clôture ${e.triggerKey} : vérifier rejet + reprise de pression vendeuse.`,shortWhy,'rejection'):''}
 ${cardAllowed('breakdown')?scenarioCard('Cassure baissière','🔴',e.breakdown,'short',`clôture de ${e.triggerKey} sous ${axisPrice(e.breakdown.entry)} avec confirmation.`,shortNext,shortWhy,'breakdown'):''}
...
let scenarioMonitorTimer=null,scenarioMonitorSeq=0,scenarioMonitorFrames={},scenarioMonitorBar='15m',scenarioMonitorKind=null,scenarioMonitorReturn='detail',graphLiveTimer=null,detailLiveTimer=null;
...
function retainJournal(a){const terminal=new Set(a.filter(x=>!['FORMING','ACTIVATED'].includes(x.status)).slice(-1500));return a.filter(x=>['FORMING','ACTIVATED'].includes(x.status)||terminal.has(x))}
function saveScenarioJournal(a){return persistJSON(STORAGE.learning,retainJournal(a))}
...
function journalAdvance(lock,side,bars,activated,barMs,now=Date.now(),decisionAvailable=true){
 ...
 if(rec.status==='FORMING'){
   const after=complete.filter(b=>b.t+barMs>Math.max(rec.createdAt,rec.lastEvaluatedClose||0));
   let cursor=rec.lastEvaluatedClose||rec.createdAt;
   const uncertain=reason=>{rec.status='UNVERIFIED';rec.unverifiedReason=reason;};
   for(const bar of after){
     const close=bar.t+barMs,stop=side==='short'?bar.h>=lock.stop:bar.l<=lock.stop;
     const entryPossible=side==='short'?bar.l<=lock.entry:bar.h>=lock.entry;
     const closeBeyond=side==='short'?bar.c<=lock.entry:bar.c>=lock.entry;
     if(close-cursor>barMs){uncertain('Historique FORMING incomplet');break}
   ...
   if(after.length&&!saveScenarioJournal(j))return loadScenarioJournal().find(x=>x.id===lock.id)||null;
   return rec;
 }
 if(rec.status!=='ACTIVATED'||!Number.isFinite(rec.activatedBarTs))return rec;
 if(complete.length&&complete[0].t>Math.max(rec.activatedBarTs,rec.lastProcessedBarTs||0)+barMs){rec.status='UNVERIFIED';rec.unverifiedReason='Bougies manquantes pendant le suivi';return saveScenarioJournal(j)?rec:loadScenarioJournal().find(x=>x.id===lock.id)}
 ...
 return saveScenarioJournal(j)?rec:loadScenarioJournal().find(x=>x.id===lock.id);
}
...
function learningStats(){
 ...
 return {total:j.length,closed:closed.length,legacy:j.filter(x=>x.status==='CLOSED'&&x.provenance?.type!=='imported'&&!verifiedObservation(x)).length,open:j.filter(x=>x.status==='FORMING'||x.status==='ACTIVATED').length,groups,journal:j}
}
...
function createStateBackup(){return {schema:'IR_BACKUP_V1',version:1,app:APP_VERSION,exportedAt:new Date().toISOString(),data:{scan:history,locks:scenarioLocks,favorites,journal:loadScenarioJournal(),imports:recordMap(STORAGE.imports)}}}
function exportLearningJson(){const payload=createStateBackup();const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='institutional-radar-backup-'+APP_VERSION.toLowerCase().replace(/[^0-9a-z]+/g,'-')+'-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
...
function stableImportJSON(value){return JSON.stringify(value,(_,v)=>v&&typeof v==='object'&&!Array.isArray(v)?Object.fromEntries(Object.keys(v).sort().map(k=>[k,v[k]])):v)}
function importHash(text){let h=2166136261;for(let i=0;i<text.length;i++){h^=text.charCodeAt(i);h=Math.imul(h,16777619)}return (h>>>0).toString(16)}
function importBody(r){const body={...r,id:r.provenance?.originId||r.id};delete body.provenance;return body}
function importMap(value,name){if(!value||typeof value!=='object'||Array.isArray(value)||Object.keys(value).some(k=>['__proto__','constructor','prototype'].includes(k)))throw Error('Sauvegarde invalide : '+name);return value}
function validateStateBackup(obj){
 if(obj.schema!=='IR_BACKUP_V1'||obj.version!==1)throw Error('Version de sauvegarde non prise en charge');
 const d=importMap(obj.data,'data');validateLearningImport(d.journal);
 for(const key of ['scan','locks','favorites','imports'])importMap(d[key]||{},key);
 const assetId=id=>typeof id==='string'&&/^[A-Z0-9][A-Z0-9_-]*$/.test(id),kinds=['breakout','pullback','breakdown','rejection'];
 for(const [id,r] of Object.entries(d.scan||{}))if(!assetId(id)||!r||!Array.isArray(r.samples))throw Error('Historique de scan invalide');
 for(const [key,lock] of Object.entries(d.locks||{})){
  const [id,kind,...extra]=key.split('::');if(!assetId(id)||extra.length||!kinds.includes(kind)||!lock||typeof lock.id!=='string'||(lock.kind!=null&&lock.kind!==kind)||(lock.direction!=null&&lock.direction!==scenarioDirection(kind)))throw Error('Verrou invalide');
  if(!scenarioValid(kind,lock,(lock.entry+lock.stop)/2))throw Error('Niveaux du verrou invalides');
 }
 for(const [id,f] of Object.entries(d.favorites||{})){
  if(!assetId(id)||f?.asset?.id!==id)throw Error('Favori invalide');
  for(const [kind,ref] of Object.entries(importMap(f.scenarios||{},'scenarios')))if(!kinds.includes(kind)||typeof ref!=='string')throw Error('Lien favori invalide');
 }
 return d;
}
function restoreStateImport(obj){
 const full=obj?.schema==='IR_BACKUP_V1';if(obj?.schema?.startsWith('IR_BACKUP')&&!full)throw Error('Version de sauvegarde non prise en charge');
 const d=full?validateStateBackup(obj):{journal:validateLearningImport(obj),scan:{},locks:{},favorites:{},imports:{}};
 const journal=loadScenarioJournal(),ids=new Map(),conflicts=[],imports=recordMap(STORAGE.imports);let added=0;
 for(const incoming of d.journal){
  const body=importBody(incoming),canonical=stableImportJSON(body);
  let prior=journal.find(r=>r.provenance?.type==='imported'&&stableImportJSON(importBody(r))===canonical);
  if(!prior){let id='imported:'+body.id+':'+importHash(canonical),suffix=0;while(journal.some(r=>r.id===id))id='imported:'+body.id+':'+importHash(canonical)+':'+(++suffix);
   prior={...body,id,provenance:{type:'imported',originId:body.id,importedAt:Date.now()}};journal.push(prior);added++;
  }
  ids.set(incoming.id,prior.id);
  if(journal.some(r=>r.id===incoming.id&&r.provenance?.type!=='imported'))conflicts.push('Observation '+incoming.id+' conservée localement');
 }
 const locks={...scenarioLocks},nextFavorites={...favorites},scan={...history};
 const merge=(target,key,value,label)=>{if(Object.hasOwn(target,key)){if(stableImportJSON(target[key])!==stableImportJSON(value))conflicts.push(label+' '+key+' conservé localement')}else target[key]=value};
 for(const [key,r] of Object.entries(d.locks||{})){const mapped=ids.get(r.id);if(!mapped){conflicts.push('Verrou '+key+' archivé : observation absente');continue}merge(locks,key,{...r,id:mapped},'Verrou')}
 for(const [id,f] of Object.entries(d.favorites||{})){const scenarios=Object.fromEntries(Object.entries(f.scenarios||{}).map(([kind,ref])=>[kind,ids.get(ref)||ref]));merge(nextFavorites,id,{...f,scenarios},'Favori')}
 for(const [id,h] of Object.entries(d.scan||{}))merge(scan,id,h,'Historique');
 // Keep complete incoming versions in the archive. Collision suffixes are checked by content;
 // the hash is an identity aid, never a signature or proof of market authenticity.
 const archive=(record,base)=>{const text=stableImportJSON(record);let key=base||'archive:'+importHash(text),n=0;while(imports[key]&&stableImportJSON(imports[key])!==text)key=(base||'archive:'+importHash(text))+':'+(++n);imports[key]=record};
 for(const [key,r] of Object.entries(d.imports||{}))archive(r,key);
 archive({schema:'IR_IMPORT_ARCHIVE_V1',data:{journal:d.journal,locks:d.locks||{},favorites:d.favorites||{},scan:d.scan||{}}});
 if(!storageTransaction({[STORAGE.learning]:retainJournal(journal),[STORAGE.locks]:locks,[STORAGE.favorites]:nextFavorites,[STORAGE.scan]:scan,[STORAGE.imports]:imports}))throw Error('Stockage indisponible : import non appliqué');
 Object.assign(scenarioLocks,locks);favorites=nextFavorites;history=scan;
 return {imported:added,conflicts:conflicts.length,details:conflicts};
}
function importedObservationStats(){const imported=loadScenarioJournal().filter(r=>r.provenance?.type==='imported'),closed=imported.filter(r=>r.status==='CLOSED'&&Number.isFinite(r.outcome?.r));return {total:imported.length,closed:closed.length,declaredAverageR:closed.length?closed.reduce((a,r)=>a+r.outcome.r,0)/closed.length:null}}
function importLearningJson(){const input=document.createElement('input');input.type='file';input.accept='.json,application/json';input.onchange=async()=>{const file=input.files?.[0];if(!file)return;try{const result=restoreStateImport(JSON.parse(await file.text()));alert(`${result.imported} observations importées, exclues des résultats vérifiés. ${result.conflicts} conflit(s) : versions locales conservées, versions importées archivées.`);toolPage('memory')}catch(e){alert('Import impossible : '+e.message)}};input.click()}
...
function chooseFreshScenario(e,preferred,direction=null){
 const base=(e?.directional?.shortScore||0)>(e?.directional?.longScore||0)?['rejection','breakdown','breakout','pullback']:['breakout','pullback','rejection','breakdown'];
 for(const kind of preferred?[preferred]:base){if(direction&&scenarioDirection(kind)!==direction)continue;if(scenarioKindAllowed(e,kind)&&scenarioValid(kind,scenarioLevelsFromEngine(e,kind),e.live))return kind}
 return null;
}

async function openScenarioMonitor(id,kind){
 const page=leavePage();
 current=all.find(x=>x.id===id)||(current?.id===id?current:null);if(!current)return;
 ...
 scenarioMonitorFrames={};scenarioMonitorBar='15m';scenarioMonitorKind=kind;
...
async function resetScenarioMonitor(kind){if(!clearScenarioLock(current.id,kind))return;scenarioMonitorKind=kind;scenarioMonitorFrames={};await renderScenarioMonitor(kind,true)}
let scenarioChartSnapshot=null,scenarioChartFrames={},scenarioChartRequestSeq=0;
function chartMath(){return {ema,rsi,stochRsi,supertrend}}
function chartJournalEvents(record){return [...(Number.isFinite(record?.activatedAt)?[{t:record.activatedAt,label:'Activation observée'}]:[]),...(Number.isFinite(record?.outcome?.ts)?[{t:record.outcome.ts,label:'Résolution observée · '+record.outcome.status}]:[])]}
async function setScenarioMonitorBar(kind,bar){
 if(!RadarChartCore.intervals[bar])return;scenarioMonitorBar=bar;renderScenarioChartOnly();await refreshScenarioChartData();
}
function scenarioChartContentHtml(){
 const s=scenarioChartSnapshot;if(!s)return '';
 const tfBtns=Object.keys(RadarChartCore.intervals).map(b=>`<button class="smallbtn ${scenarioMonitorBar===b?'active':''}" data-scenario-bar="${b}" onclick="setScenarioMonitorBar('${s.kind}','${b}')">${b}</button>`).join('');
 return `<div class="rangeBtns scenarioTfBtns">${tfBtns}</div><div id="scenarioPlot"></div><p class="sub">Niveaux verrouillés. TP2/TP3 sont des objectifs consultables ; le suivi actuel est terminal à TP1/SL. Les événements sont observés, pas des ordres exécutés.</p>`;
}
function renderScenarioChartOnly(){
 const s=scenarioChartSnapshot;if(!s)return;
 if(!document.querySelector('#scenarioPlot'))$('scenarioChartContent').innerHTML=scenarioChartContentHtml();
 document.querySelectorAll('[data-scenario-bar]').forEach(b=>b.classList.toggle('active',b.dataset.scenarioBar===scenarioMonitorBar));
 const bars=scenarioChartFrames[scenarioMonitorBar]||s.frames[scenarioMonitorBar]||[];
 paintSharedChart('scenarioPlot',{instrument:s.instrument,quote:chartQuote(s.instrument),timeframe:scenarioMonitorBar,bars,engineBars:s.references?.[scenarioMonitorBar]||[],preferEngine:true,overlays:s.overlays,events:s.events||[],structure:s.structures?.[scenarioMonitorBar]||[],ticker:{value:s.live,ts:s.tickerTs,fresh:s.tickerFresh},asOf:s.asOf});
}
async function refreshScenarioChartData(){
 const s=scenarioChartSnapshot;if(!s)return;const page=pageRevision,seq=++scenarioChartRequestSeq,bar=scenarioMonitorBar;
 try{const data=await candles(s.instrument,bar,RadarChartHost.policy.minimumHistory);if(page!==pageRevision||seq!==scenarioChartRequestSeq||bar!==scenarioMonitorBar||!scenarioChartSnapshot||s.instrument!==scenarioChartSnapshot.instrument)return;scenarioChartFrames[bar]=data;renderScenarioChartOnly()}
 catch(err){if(page===pageRevision&&seq===scenarioChartRequestSeq)sharedChartViews.get('scenarioPlot')?.controller.error('Consultation indisponible · '+err.message)}
}
function resetScenarioChartView(){sharedChartViews.get('scenarioPlot')?.controller.panel?.action('reset')}
function zoomScenarioMonitor(kind,delta){sharedChartViews.get('scenarioPlot')?.controller.panel?.action(delta<0?'zoomIn':'zoomOut')}
function scenarioBiasCompatible(e,kind){
 if(!isShortScenarioKind(kind))return !!e?.biasBull;
 return !!(e?.shortSetup&&(kind==='rejection'?['rejection','reversal'].includes(e.shortPattern):e.shortPattern==='breakdown'));
}
function monitorTriggerStatus(v){
 ...
 if(v.invalid)return {state:'yellow',label:'ALERTE PROVISOIRE · INVALIDATION TOUCHÉE',reason:`Prix au-delà de ${axisPrice(v.stop)} ; le journal attend la bougie clôturée pour statuer.`};
...
async function renderScenarioMonitor(kind,initial=false){kind=scenarioMonitorKind||kind;const page=pageRevision,seq=++scenarioMonitorSeq;const abandoned=()=>page!==pageRevision||seq!==scenarioMonitorSeq;try{
 if(initial){
  scenarioChartFrames={};scenarioChartRequestSeq++;
  ...
  const entries=await Promise.all(DECISION_SPECS.map(async x=>[x[0],await safeLoad(x[0],x[1])]));
 ...
 if(abandoned())return;
 // Business cadence remains the baseline default 15m + locked trigger, independently of the inspected timeframe.
 const refreshBars=new Set(['15m']);if(scenarioLocks[scenarioLockKey(current.id,kind)]?.triggerKey)refreshBars.add(scenarioLocks[scenarioLockKey(current.id,kind)].triggerKey);
 ...
 const overlays=[{key:'entry',value:sc.entry,color:'#65b8ff',label:'Déclencheur / entrée'},{key:'stop',value:sc.stop,color:'#ff6974',label:'SL / invalidation'},{key:'tp1',value:sc.tp1,color:'#45dc7a',label:'TP1'},{key:'tp2',value:sc.tp2,color:'#45dc7a',label:'TP2'},{key:'tp3',value:sc.tp3,color:'#45dc7a',label:'TP3'}];
 scenarioChartSnapshot=RadarChartCore.immutable({instrument:marketInstId,frames:scenarioMonitorFrames,kind:displayKind,lock:sc,live,state,overlays,tickerFresh,tickerTs,analysisAt:scenarioMonitorAnalysisAt,asOf:Date.now(),references:Object.fromEntries(Object.entries(e.F||{}).map(([tf,f])=>[tf,f?.cs||[]])),structures:Object.fromEntries(Object.entries(e.F||{}).map(([tf,f])=>[tf,RadarChartHost.structure(f,tf)])),events:chartJournalEvents(jrec)});
 ...
 const preservedSimulator=activeSimulator?.key===scenarioSimKey(sc,side)?document.querySelector('.scenarioSimulator'):null;
 const preservedChart=!initial?document.querySelector('#scenarioChartContent'):null;
 $('deepBody').innerHTML=`<button class="btn secondary" onclick="closeScenarioMonitor()">← Scénarios</button><div class="panel"><div class="monitorHero"><div class="monitorState"><div><div class="monitorHeadline">${current.sym} • ${displayKind==='breakout'?'Cassure conditionnelle':displayKind==='pullback'?'Rebond sur support':displayKind==='rejection'?'Rejet conditionnel de résistance':'Cassure baissière'}</div><div class="monitorSub">🔒 ${scenarioActionLabel(displayKind)} • ${esc(marketInstId)} • scénario verrouillé • ${anchorKey} = ancrage • ${triggerKey} = déclenchement • ${tickerFresh?'🟢 Prix actualisé':'🟠 Prix non actualisé'}${tickerTs?' · ticker '+clockStamp(tickerTs):''} • actualisation ~10s • bougie visible rechargée</div></div><span class="monitorLight light${state.charAt(0).toUpperCase()+state.slice(1)}"></span></div><div style="margin-top:12px"><div class="monitorHeadline">${label}</div><div class="monitorSub">${tickerFresh?'Prix actualisé':'Dernier prix connu'} : ${axisPrice(live)} • ${esc(reason)}</div></div>${action}${favoriteActions}</div>${levels}</div><div class="panel monitorChart"><h2>📈 Projection du scénario</h2><div class="sub">Les lignes pointillées sont les niveaux verrouillés. Les bougies et le prix se mettent à jour ; les niveaux du scénario ne bougent pas.</div><div id="scenarioChartContent">${scenarioChartContentHtml()}</div></div>${scenarioSimPanel(sc,side)}<div class="panel"><h2>🔎 Pourquoi cet état ?</h2><div class="signalSummary"><div class="signalCount signalGood"><b>${e.signalCounts.positive}</b><span>favorables</span></div><div class="signalCount signalBad"><b>${e.signalCounts.negative}</b><span>défavorables</span></div><div class="signalCount signalWarn"><b>${e.signalCounts.warning}</b><span>à surveiller</span></div><div class="signalCount signalNeutral"><b>${e.signalCounts.neutral}</b><span>neutres</span></div></div><div class="signalList">${e.signals.map(x=>{let cls=x.status==='positive'?'sigPos':x.status==='negative'?'sigNeg':x.status==='warning'?'sigWarn':'sigNeu';let lab=x.status==='positive'?'FAVORABLE':x.status==='negative'?'DÉFAVORABLE':x.status==='warning'?'À SURVEILLER':'NEUTRE';return `<div class="signalRow"><div class="sigText"><b>${esc(x.name)}</b><small>${esc(x.detail)} • ${esc(x.family)}</small></div><span class="sigBadge ${cls}">${lab}</span></div>`}).join('')}</div></div>`;if(preservedSimulator){document.querySelector('.scenarioSimulator').replaceWith(preservedSimulator);if(focusedSimInput)$(focusedSimInput)?.focus({preventScroll:true})}else bindScenarioSim(sc,side);if(preservedChart)document.querySelector('#scenarioChartContent').replaceWith(preservedChart);renderScenarioChartOnly();refreshScenarioChartData();
...
async function metricPage(metric){
 if(metric==='Prix')return graphPage(1);
 const page=pageRevision,instrument=activeInstrumentId(),asset=current,hasTimeframe=['Volume','Momentum'].includes(metric),selection={selectedTs:null};
 let bar='1H',days=1,loadSeq=0;
 async function load(){
  const seq=++loadSeq;let body='',values=[],times=[],formatter=String,desc='',options={};
  try{
   if(hasTimeframe){
    const limit=Math.min(1800,Math.max(2,Math.ceil(days*86400000/timeframeMs(bar))+(metric==='Momentum'?1:0))),cs=await candles(instrument,bar,limit);
    if(metric==='Volume'){values=cs.map(x=>x.v);times=cs.map(x=>x.t);formatter=v=>money(v).replace('$',chartQuote(instrument));desc=`Volume de cotation par bougie ${bar} · ${chartQuote(instrument)}.`;options={kind:'histogram',interval:timeframeMs(bar)};}
    else{values=cs.slice(1).map((x,i)=>(x.c-cs[i].c)/cs[i].c);times=cs.slice(1).map(x=>x.t);formatter=v=>(v*100).toFixed(2)+' %';desc=`Variation signée d’une clôture ${bar} à la suivante. La bougie en formation reste provisoire.`;options={kind:'line',signed:true,interval:timeframeMs(bar)};}
   }else if(metric==='Funding'){
    if(!asset.perpId)throw Error('Pas de contrat perpétuel associé : pas d’historique de funding.');
    const rows=await get('/public/funding-rate-history?instId='+encodeURIComponent(asset.perpId)+'&limit=100'),cut=Date.now()-days*86400000;
    const observed=rows.map(x=>({ts:nullableNumber(x.fundingTime??x.ts),value:nullableNumber(x.fundingRate)})).filter(x=>x.ts!=null&&x.ts>=cut).sort((a,b)=>a.ts-b.ts);
    values=observed.map(x=>x.value);times=observed.map(x=>x.ts);formatter=v=>(v*100).toFixed(4)+' %';desc=`Événements de funding OKX · ${asset.perpId} · couverture limitée aux 100 événements renvoyés, sans interpolation.`;options={kind:'histogram',signed:true};
   }else{
    const observed=(history[asset.id]?.samples||[]).filter(x=>x.ts>=Date.now()-days*86400000),field=metric==='OI'?'oi':'score';
    values=observed.map(x=>x[field]);times=observed.map(x=>x.ts);formatter=metric==='OI'?money:v=>String(v)+'/100';desc=`Observations locales des scans · ${metric==='OI'?'Open Interest en USD':'score de confluence'} · les espaces représentent le temps écoulé, sans interpolation.`;options={kind:'scatter'};
   }
   const observed=RadarChartMetrics.observations(values,times),valid=observed.filter(x=>x.value!=null),first=valid[0]?.t,last=valid.at(-1)?.t;
   const definitions={OI:'OI = Open Interest : valeur notionnelle des contrats encore ouverts. Sa variation ne donne pas à elle seule le sens des positions.',Volume:'Le volume mesure l’activité échangée. Un mouvement se lit avec le prix, la structure et les autres données.',Funding:'Paiement périodique entre positions longues et courtes. Positif : les longs paient généralement les shorts ; négatif : l’inverse.',Momentum:'Variation signée entre deux clôtures consécutives. Une valeur positive indique une hausse, une valeur négative une baisse.',Score:'Confluence interne pour prioriser les actifs, pas une probabilité de gain. Un score de zéro est une observation valide.'};
   body=`<button class="btn secondary" onclick="backDetail()">← ${esc(asset.sym)}</button><div class="panel"><h2>📊 ${esc(metric)} — ${esc(asset.sym)}</h2><div class="sub">${esc(desc)}</div>${rangeControls(bar,days,hasTimeframe)}<div id="metricMeta" class="sub">${hasTimeframe?bar+' · ':''}${days} jour(s) demandé(s) · ${valid.length} observations disponibles · ${first!=null?clockStamp(first)+' → '+clockStamp(last):'couverture indisponible'} · consultation ${clockStamp(Date.now())}</div><button class="smallbtn" id="metricRefresh" type="button">↻ Actualiser ce graphique</button><div id="metricPlot"></div></div><div class="panel"><h2>🧠 Lecture simple</h2><p>${definitions[metric]}</p><p>Prix + volume + OI + funding + structure : aucune décision à partir de ce seul graphique.</p></div>`;
  }catch(e){body=`<button class="btn secondary" onclick="backDetail()">← ${esc(asset.sym)}</button><div class="panel" role="status">Données indisponibles : ${esc(e.message)}<button class="smallbtn" id="metricRefresh" type="button">↻ Réessayer</button></div>`;}
  if(page!==pageRevision||seq!==loadSeq)return;
  activeMetricPlot?.dispose();activeMetricPlot=null;$('deepBody').innerHTML=body;
  const host=document.querySelector('#metricPlot');if(host)activeMetricPlot=RadarChartMetrics.mount(host,values,times,{...options,formatter,label:metric+(metric==='Volume'?' · '+chartQuote(instrument)+' / '+bar:''),state:selection});
  $('metricRefresh').onclick=load;
  document.querySelectorAll('#bars button').forEach(b=>b.onclick=()=>{bar=b.dataset.bar;load()});
  document.querySelectorAll('#ranges button').forEach(b=>b.onclick=()=>{days=+b.dataset.days;load()});
 }
 await load();
}
...
function backDetail(){leavePage();stopScenarioUpdates();stopCandleStream();if(graphLiveTimer){clearInterval(graphLiveTimer);graphLiveTimer=null}if(scenarioMonitorTimer){clearInterval(scenarioMonitorTimer);scenarioMonitorTimer=null}$('deep').classList.add('hidden');$('detail').classList.remove('hidden');return detailAsync()}
...
function clearScanHistory(){if(!recoverStorageTransaction())return;try{for(const key of [STORAGE.scan,'ir_hist_v63','ir_hist_v52'])localStorage.removeItem(key)}catch(_){storageFailure();return}history={};alert('Historique local des scans effacé. Relance le scan.')}
...
async function runWalkForwardLab(){
  ...
  try{
    ...
    const rows=[],coverage={evaluated:0,complete:0,incomplete:0};
    for(const [phase,a,b] of phases){
      for(let i=Math.max(80,a);i+18<b;i+=Math.max(1,Math.floor((b-a)/18))){
        ...
        coverage.evaluated++;if(specs.every(([tf])=>frames[tf].length>=40))coverage.complete++;else coverage.incomplete++;
        const px=btClose(base[i]);const prev=base[Math.max(0,i-24)],dayMove=btClose(prev)>0?(px/btClose(prev)-1)*100:null,historyWindow=base.slice(Math.max(0,i-24),i+1),low24=Math.min(...historyWindow.map(c=>Number(c[3]))),high24=Math.max(...historyWindow.map(c=>Number(c[2])));const historical={id:inst,decisionAt:ts,perpId:inst.endsWith('-SWAP')?inst:(current?.perpId||null),price:px,chg:dayMove,rangePos:high24>low24?(px-low24)/(high24-low24):null,vol:Number(base[i][7])||null,oi:null,oiDelta:null,funding:null};const e=adaptiveEngine(frames,historical);
        if(!e)continue;
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=8.9.17-rc.1',{updateViaCache:'none'}).catch(console.warn));
