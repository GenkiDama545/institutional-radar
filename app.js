...
function score(x){
  ...
  // V8.6.4 — ranking calibration: a configuration must be both technically coherent
  // and directionally usable. The directional engine therefore contributes to the
  // final tradability score instead of being used only as a filter.
  const d=x?.directional||null;
  if(d){
    const ds=Math.max(Number(d.longScore)||0,Number(d.shortScore)||0);
    const eligible=(d.longEligible||d.shortEligible);
    if(eligible)s=Math.round(s*.72+ds*.28);
    else s=Math.min(s,54);
  }
...
function adaptiveEngine(frames,currentData){
 ...
 const trigger=F[triggerKey], nearR=trigger.distR<=Math.max(trigger.atr*1.2,live*.004),nearS=trigger.distS<=Math.max(trigger.atr*1.2,live*.004), shortNearR=trigger.distR<=Math.max(trigger.atr*1.5,live*.006), shortNearS=trigger.distS<=Math.max(trigger.atr*1.5,live*.006);
 ...
 // V8.6.4 — LONG and SHORT are evaluated as two independent engines.
...
function exportLearningJson(){const payload={schema:'IR_LEARNING_V1',exportedAt:new Date().toISOString(),app:'Institutional Radar V8.6.4',journal:loadScenarioJournal()};const blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='institutional-radar-learning-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
