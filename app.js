...
async function scan(){
 if(scanRunning)return;scanRunning=true;$('scan').disabled=true;
 ...
 try{
   ...
   raw.sort((a,b)=>b.score-a.score);raw.forEach(x=>x.bucket=bucket(x.score));localStorage.setItem(STORAGE.scan,JSON.stringify(history));all=raw;$('status').textContent=`${all.length} Spot actifs • ${all.filter(x=>x.analysisCoverage==='complete').length} analysés entièrement • ${shortScanErrors} incomplets`;render();
 }catch(e){$('status').textContent='Erreur API — réessaie : '+String(e.message||e);console.error(e)}finally{scanRunning=false;$('scan').disabled=false}
}
