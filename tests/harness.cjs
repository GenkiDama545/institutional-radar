...
function env(initial={}){
 ...
 const node=id=>nodes[id]??=( {innerHTML:'',textContent:'',value:'',disabled:false,dataset:{},classList:{s:new Set(),add(x){this.s.add(x)},remove(x){this.s.delete(x)},contains(x){return this.s.has(x)},toggle(x){this.s.has(x)?this.s.delete(x):this.s.add(x)}},setAttribute(){},querySelectorAll(){return []}});
