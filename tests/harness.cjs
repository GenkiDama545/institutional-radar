...
require('node:test').afterEach(()=>{for(const dom of environments)dom.window.close();environments.clear()});
function env(initial={},appSource=null){
 ...
 const app=appSource??fs.readFileSync(path.join(root,'app.js'),'utf8');vm.runInContext(app.slice(0,app.indexOf("document.querySelectorAll('#marketMode")),ctx,{filename:'app.js'});
