...
test('release: all versioned entry assets exist and share the application/service-worker version',()=>{
 ...
 assert.ok(version.includes('-rc.'),'candidate must have a distinct release name');assert.match(html,new RegExp('<title>Institutional Radar V'+version.replace(/[.]/g,'\\.')+'</title>'));
